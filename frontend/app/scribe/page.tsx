...
export default async function ScribeShowsPage() {
  const session = await auth();
  const role = (session?.user as any)?.role;

  if (!session || role !== 'SCRIBE') {
    redirect('/');
  }
  ...
  return (
    <main className="max-w-2xl mx-auto p-4 md:p-6">
      <div className="mb-6 mt-2">
        <h2 className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>My Shows</h2>
        <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>Shows you are assigned to score</p>
      </div>
