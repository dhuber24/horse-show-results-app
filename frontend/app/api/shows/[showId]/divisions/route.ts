...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ showId: string }> }) {
  ...
  const { json, status } = await safeFetchBackend(`${API_URL}/shows/${showId}/divisions/`, { headers, cache: 'no-store' });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ showId: string }> }) {
  ...
  const url = isBulk
    ? `${API_URL}/shows/${showId}/divisions/bulk`
    : `${API_URL}/shows/${showId}/divisions/`;
