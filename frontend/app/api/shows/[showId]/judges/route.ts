...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ showId: string }> }) {
  ...
  const { json, status } = await safeFetchBackend(`${API_URL}/shows/${showId}/judges/`, { headers, cache: 'no-store' });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ showId: string }> }) {
  ...
  const { json, status } = await safeFetchBackend(`${API_URL}/shows/${showId}/judges/`, {
    method: 'POST',
    headers,
