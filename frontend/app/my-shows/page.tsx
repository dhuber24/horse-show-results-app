...
function ShowBillCard({ show }: { show: MyShow }) {
  ...
  const { bill } = show;
  const hasCharges = bill.total_cents > 0;
...
function ReservationLine({ line }: { line: BillReservationLineProps }) {
  return (
    <>
      <dt>
        {line.label}
...
type BillReservationLineProps = MyShow['bill']['reservation_lines'][number];
