...
export default function HorseColorForm({ color }: { color?: HorseColor }) {
  const router = useRouter();
  const isEdit = !!color?.id;
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    setError(null);
    const url = isEdit ? `/api/horse-colors/${color!.id}` : '/api/horse-colors';
    ...
    if (res.ok) {
      if (isEdit) {
        ...
        router.push('/admin/horses/colors');
        router.refresh();
      }
    } else {
      const err = await res.json().catch(() => ({}));
      setError(err.detail ?? `Failed to ${isEdit ? 'update' : 'create'} color.`);
    }
  };

  const handleDelete = async () => {
    setDeleting(true);
    const res = await fetch(`/api/horse-colors/${color!.id}`, { method: 'DELETE' });
    setDeleting(false);
    if (res.ok) {
      router.push('/admin/horses/colors');
    } else {
      const err = await res.json().catch(() => ({}));
      setError(err.detail ?? 'Failed to delete color. It may be in use by existing horses.');
    }
  };
