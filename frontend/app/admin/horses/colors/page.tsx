import Link from 'next/link';
import { fetchHorseColors } from '@/lib/api';
import HorseColorForm from './HorseColorForm';
import Breadcrumbs from '@/components/Breadcrumbs';

export default async function AdminHorseColorsPage() {
  const colors = await fetchHorseColors();

  return (
    <main className="max-w-3xl mx-auto p-4 md:p-6 space-y-8">
      <div>
        <Breadcrumbs crumbs={[
          { label: 'Admin', href: '/admin' },
          { label: 'Horses', href: '/admin/horses' },
          { label: 'Colors' },
        ]} />
        <h1 className="text-2xl font-bold mt-2" style={{ color: 'var(--foreground)' }}>Horse Colors</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
          Manage the color options available when adding or editing a horse.
        </p>
      </div>

      <section>
        <h2 className="text-lg font-semibold mb-3" style={{ color: 'var(--foreground)' }}>Add Color</h2>
        <HorseColorForm />
      </section>

      <section>
        <h2 className="text-lg font-semibold mb-3" style={{ color: 'var(--foreground)' }}>
          All Colors
          <span className="ml-2 text-sm font-normal" style={{ color: 'var(--muted)' }}>
            ({colors.length})
          </span>
        </h2>
        {colors.length === 0 ? (
          <p style={{ color: 'var(--muted)' }}>No colors yet.</p>
        ) : (
          <ul className="space-y-2">
            {colors.map((c: any) => (
              <li key={c.id}>
                <Link
                  href={`/admin/horses/colors/${c.id}`}
                  className="flex items-center justify-between p-4 rounded-lg border transition-colors hover:bg-amber-50"
                  style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}
                >
                  <div>
                    <div className="font-semibold" style={{ color: 'var(--foreground)' }}>{c.name}</div>
                    <div className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>Sort: {c.sort_order}</div>
                  </div>
                  <span className="text-sm ml-4 shrink-0" style={{ color: 'var(--accent)' }}>Edit →</span>
                </Link>
              </li>
            ))}
          </ul>
        )}
      </section>
    </main>
  );
}
