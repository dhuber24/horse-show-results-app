    ...
    <main className="max-w-5xl mx-auto p-4 md:p-6">
      ...
      <div className="p-5 rounded-lg border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-4">
          <h2 className="text-lg font-semibold" style={{ color: 'var(--foreground)' }}>All Users</h2>
          {users.length > 0 && (
            <span className="text-xs" style={{ color: 'var(--muted)' }}>
              {(['ADMIN', 'SHOW_MANAGER', 'SHOW_SECRETARY', 'SCRIBE', 'GATE_STEWARD', 'JUDGE', 'EXHIBITOR', 'TRAINER'] as const)
                .map(role => {
                  const count = users.filter((u: any) => u.role === role).length;
                  if (count === 0) return null;
                  const labels: Record<string, string> = { ADMIN: 'Admin', SHOW_MANAGER: 'Manager', SHOW_SECRETARY: 'Secretary', SCRIBE: 'Scribe', GATE_STEWARD: 'Gate Steward', JUDGE: 'Judge', EXHIBITOR: 'Exhibitor', TRAINER: 'Trainer' };
