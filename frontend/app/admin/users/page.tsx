        ...
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-4">
          ...
          {users.length > 0 && (
            <span className="text-xs" style={{ color: '#8b7355' }}>
              {(['ADMIN', 'SHOW_MANAGER', 'SHOW_SECRETARY', 'SCOREKEEPER', 'GATE_STEWARD', 'EXHIBITOR', 'TRAINER'] as const)
                .map(role => {
                  const count = users.filter((u: any) => u.role === role).length;
                  if (count === 0) return null;
                  const labels: Record<string, string> = { ADMIN: 'Admin', SHOW_MANAGER: 'Manager', SHOW_SECRETARY: 'Secretary', SCOREKEEPER: 'Scorekeeper', GATE_STEWARD: 'Gate Steward', EXHIBITOR: 'Exhibitor', TRAINER: 'Trainer' };
