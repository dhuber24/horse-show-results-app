...
- **Health flags** (`GET /shows/{id}/health-flags`): not a table. Which entered horses have health paperwork that will not carry them through the show, derived on read from `horse_documents` and judged against `shows.end_date`. Computed rather than stored so it clears itself when the exhibitor uploads a current document — there is no row to remember to close. Surfaced on `/admin/shows/[id]/entries`, on the check-in checklist per horse, and (as a warning) on the exhibitor's registration screen.
...
- **Health paperwork does not gate entry.** A missing, undated, or lapsed Coggins used to 422 on both entry paths; it now flags. Refusing the entry never made a horse compliant — it just moved the discovery to the desk and turned the office's to-do into an "override" that recorded a bypass. Do not re-add the block, and do not add a `show_verifications` kind for Coggins: the desk cannot attest to a document that has expired, and one that has not needs no attesting.
