...
- Show Manager registration at `/register/show-manager` and Show Secretary registration at `/register/show-secretary` collect a name, an email, a password and an optional **Company / Organization**. Both redirect to `/welcome` once signed in.
- **Every show manager and secretary lands in a show company** (migration 143, `show_companies.place_in_company`), because a paid feature can only reach an account through one. The Company / Organization box decides which: **blank** gives them a company of their own, named after them (`show_companies.owner_user_id`); **a name nobody has used** creates that organization with them in it; **a name that already exists** (matched ignoring case and spacing) is a **join request, never a membership** — they get their own company meanwhile, and the organization's page shows them under *Asked to join* for a GaitDesk admin to add or decline. Joining by typing a name would sell a club's subscription to anybody who knew what it was called. Admin-created accounts and role changes into the show office get their own company through `_ensure_role_profile`.
- `/welcome` is the post-signup questionnaire, and it exists because these questions need a session to answer. It asks which associations the person is certified with — breed registries and clubs in one list, each with an optional ID number — and whether they also compete. Every question is skippable, nothing marks it done, and it can be reopened later.
- **Certifications are recorded, never verified.** `PUT /users/me/certifications` stores what the caller ticked in `show_secretary_certifications` (`GET` reads it back) and refuses nothing but an association id that does not exist. GaitDesk holds no standing with APHA or anyone else, so it cannot tell a real certification from a typo — the same call `exhibitor_registrations` makes about a membership card.
- Both screens previously carried an APHA certification lookup keyed on the email being registered, and the secretary's **blocked submission** without a live hit. That is gone. It refused registrations over a fact this app cannot establish, it turned away secretaries whose email on APHA's list differed from their signup email, and naming one registry on the only screen every new staff member sees misrepresented an app that also serves AQHA, ApHC, FQHR and unaffiliated shows.
- The picker those screens rendered could never work either: it fetched `/api/associations` on mount, which requires a session that somebody creating an account does not have, so it 401'd and rendered an empty list for every person it was shown to. Asking after sign-in is what fixes it.
- **The `exhibitors` row is what grants exhibitor access, not `users.role`.** Ticking "I also compete" calls `POST /exhibitors/me`, which is role-agnostic; the account keeps its `SHOW_MANAGER` or `SHOW_SECRETARY` role. The backend already worked this way — `show_registration._exhibitor_for_user` and `my_shows` both look the row up and 403 without one — so this only brought the frontend into line. Read it through `loadExhibitor()` / `canActAsExhibitor()` in `frontend/lib/exhibitor-access.ts` (memoised per request), never `role === 'EXHIBITOR'`.
...
- **A feature is switched on for a show company** — the club or firm that runs shows — and reaches every account in `show_company_members`. Not per account, because staff come and go and the business is what paid; not per show, because the show bill import is how a show comes to exist. An account in two companies has whatever either has.
