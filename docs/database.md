...
| `129_classes_entered_by_qualification.sql` | Add `classes.entered_by_qualification`, and mark the eight Grand & Reserve classes on the existing schedule. **A Grand & Reserve Champion halter class is not entered.** The first- and second-place horses from each qualifying class are called back once those classes have been judged, so at the moment an exhibitor is filling in an entry form there is nothing to sign up for — and every one of them was sitting in the class picker, priced at $0, waiting for anybody who scrolled far enough. A column rather than a name test at the point of entry, because the name is only ever a *guess* and somebody has to be able to correct it: `rules.disciplines.entered_by_qualification` seeds the value when a class is created (the same derive-then-store shape as `score_type`), the column is the authority thereafter, and the class list screen toggles it. The backfill regex mirrors `_QUALIFYING_ONLY_RE` and is narrow on purpose — "champion" alone is not enough, since a Hi-Point champion is an award rather than a class. What this does **not** do is work out who qualified: that needs a relationship between a championship and the classes that feed it, which the app does not hold. `POST /shows/{id}/register` refuses these outright (`CLASS_BY_QUALIFICATION`) and the desk still enters them, because the office is standing there when the judge calls the horses back. |
| `130_show_fees_breed_association_only.sql` | Add `show_fees.breed_association_only`, off by default. **Whether a show's own automatic charge counts only the breed association's own classes.** Found on a real show bill: two hand-built `per_horse` rows named "APHA All Day fee" (Open/Amateur/Novice and Youth), each carrying a note reading "$45 per judge x 4 APHA judges, one horse, APHA classes only... All Breed (MNSPHC or WSCA) classes... are not included" — the secretary had worked out the scope and the arithmetic by hand because nothing on the row itself could express either. `per_judge_per_entry` (migration 125) already scopes itself unconditionally, because that unit is the breed body's own assessment (SC-125.B) by definition; this column is the same choice offered to a show's *own* invented charge, which is not always shaped that way. `backend/billing.py`'s `charge_lines` now takes the raw entry list rather than precomputed counts, so it can compute a breed-scoped horse/entry count per fee row rather than once for the whole bill. Off by default so every existing fee — including the two rows above, left untouched — keeps billing exactly as it always has until a manager ticks the box in `ShowChargesEditor`. **Superseded by 131 below** — the checkbox turned out to have no real use case and was removed. |
| `131_drop_show_fees_breed_association_only.sql` | Drop `show_fees.breed_association_only` again. There was no real case for the opt-out it offered: a show with no club sanctioning set up (most shows) has no club-sanctioned classes to exclude, so the scoping is a no-op for it either way, and every real example found (the two "APHA All Day fee" rows migration 130 was built for) wanted it on. `billing.charge_lines` now scopes every automatic charge to the breed association's own classes unconditionally — the same way `per_judge_per_entry` always has — so the column, and the checkbox in `ShowChargesEditor` that set it, had nothing left to say. Same shape as `100_drop_trainer_membership_verification.sql`: a kind or column added, tried, and reversed once the real use case turned out not to need the choice. |
| `132_office_charge_becomes_a_fee.sql` | Convert `shows.office_charge_cents` / `office_charge_basis` into an ordinary `show_fees` row, then drop both columns. The office charge was the app's only non-class charge when migration 055 added it, so it lived on the show row and was hard-coded into `build_bill`. Migration 112 then added `per_exhibitor` to `show_fees.unit` and said why in as many words: `office_charge_basis = 'per_back_number'` is "a basis for *that one* charge rather than a unit any fee can carry". This completes that generalisation -- `per_back_number` maps to `per_exhibitor`, `per_horse` to `per_horse`, and `billing.charge_lines` already billed both correctly. The point is not tidiness: a column can only be edited by its own bespoke control, so the office charge had its own box, its own state, its own save button, its own function in `billing.py` and its own line in every bill payload -- while being, to an exhibitor, one more automatic charge beside a drug fee. As a fee row it is renamable, removable and added the same way as everything else. Converted rows get `sort_order = -1` so they sort where the office charge has always rendered. **One behaviour moves with it**: every automatic charge counts only the breed association's own classes (migration 131), so the converted office charge now does too -- an exhibitor entered solely in a club's All Breed classes previously owed it and now does not. That is one rule reaching one more row rather than a new exception. |
| `133_show_sanctioning_fee_unit.sql` | Give a club's sanction fee a **unit**, and rename `show_sanctioning.per_class_fee_cents` to `fee_amount_cents` now that the name is no longer the whole story. Per class entered was the only thing a club could charge (migration 072), and it is one of several ways clubs actually charge on a real show bill — an all-day fee per horse, a per-judge assessment per horse, a card fee per exhibitor. A secretary meeting one of those had the same two options they had before migration 131 scoped the automatic charges: work the arithmetic out by hand into a flat amount, or leave the money off the app. `fee_unit` takes `per_entry` (the default, and the pre-133 behaviour), `per_exhibitor`, `per_horse`, `per_judge_per_horse` or `per_judge_per_exhibitor` — the same vocabulary as `show_fees.unit`, priced through the same `billing.charge_multiplier`, because "per judge, per horse" must not mean two things depending on whose fee it is. `per_judge_per_entry` is deliberately **not** offered: that is the breed body's own per-entry assessment (APHA SC-125.B and its kin), which belongs to the show's fee catalog where migration 125 put it, and a club billing per class entered is `per_entry`. Every unit counts only the classes that club approves (`class_sanctioning`, migration 113), which is migration 131's rule read from the other side. Billing splits accordingly: `sanction_rates` returns the `per_entry` clubs, whose money rides on each class line, and `billing.sanction_charge_lines` returns the rest as one itemised charge per exhibitor; a club can only reach one of the two, and both ends filter so it cannot be charged twice. `build_bill` gains `sanction_lines` and `class_sanction_total_cents`, with `sanction_total_cents` still the sum of both so every screen reading it keeps footing. Set in setup Step 6 beside the club and its classes; the rename is guarded on the column name so the migration re-runs cleanly. |
| `139_show_marquees.sql` | New table `show_marquees` — what the marquee along the bottom of the show's live screens carries. It had only ever carried the posted results, and a show office has other things to tell a room ("Class 22 has moved to Ring 2", "Lunch until 1:00") that meant walking over to the TV. One row per show, keyed on `show_id`: `mode` (`results` default / `message` / `both`), `message` (one line, ≤500 characters by CHECK), `updated_at`, `updated_by_user_id`. **No row is `results`**, which is what every show had before. The message is **kept when the mode goes back to `results`**, so an announcement can be taken down and put back up without retyping it. **A table rather than columns on `shows`**: it changes through the day, nothing in setup asks about it, and `routers/shows.py` builds the show payload by hand, so a column there would ride on every show read in the app for one screen. Written by `PUT /shows/{id}/marquee` (show office), which refuses a message mode with nothing typed; read by the public `GET`, which reports `effective_mode` — falling back to `results` if such a row ever exists anyway, since a marquee must never scroll a blank band. Defaults and both CHECKs are stated outside the `CREATE TABLE`, and `create_all` won the race again: the dev backend hot-reloaded the model first and the migration logged `relation "show_marquees" already exists, skipping`. Backward-compatible (`CREATE TABLE`), so it goes to production before the code. |
| `140_exhibitor_office_records.sql` | Add `exhibitors.email` and `exhibitors.created_by_user_id`. Every exhibitor record had been made one of two ways -- somebody created an account, or somebody seeded test data -- so `GET /exhibitors/names` filtered to `user_id IS NOT NULL` and the desk's add-somebody form said an exhibitor "needs an account before they can be entered". That is not a thing a secretary can do for a walk-up standing at the counter with a paper entry blank. **`created_by_user_id`** records the staff member who typed the person in, the same column `horses.created_by_user_id` already carries for the same reason, and it is what tells a real accountless person apart from the accountless seed data the pickers exclude -- so the office types somebody in once rather than once a weekend. **`email`** is the address taken at the counter and **never a login**: `phone` has always lived on `exhibitors` but an email lives on `users`, which an office record has none of; once a record is linked to an account that account's address is the authority. Its second job matters later -- when the person does create an account there are two rows for one person, and matching on name alone is a guess (a name is not proof of identity), while an address the office wrote down matching the one somebody signed up with is a good enough reason to *ask*. Staff still press the button. Both columns are nullable with no default, plus partial indexes on `created_by_user_id` and `lower(email)`, so the running release ignores them and this is safe to apply before the code. |
| `141_show_bill_imports.sql` | New table `show_bill_imports` — the record of the model reading a show's own printed show bill, shaped after `document_extractions` and for the same reason: **the model suggests and a person saves**. The upload writes a `pending` row and the read runs in the background, because a twenty-page bill takes minutes; a pending row older than the read could take is reported as failed by the reader rather than by a sweeper. `extracted` is the read, `accepted` the reviewed payload the show was created from, `show_id` the show. The file is kept so creating the show can put it on file as that show's SHOWBILL document (`showbill_source` stays `generated`). Defaults and the status CHECK are stated apart from the `CREATE TABLE`, because startup's `create_all` made the table first on dev — exactly the migration-114 case. |
| `138_show_physical_document_check.sql` | Add `shows.requires_physical_document_check` (default true). Migration 097 let a show say *which* health papers it requires; it did not let the show say whether the exhibitor has to bring the original, and the desk assumed yes for every show. The two questions are ones the desk already keeps apart: `horse_documents` answers "is the date still good", and a `show_verifications` row of kind `horse_health_document` answers "did somebody at this counter hold the paper and decide it describes this horse". A show that takes the upload as sufficient is signing off on nothing, so a row per horse per document was a check nobody working that show could clear — the same failure `show_associations` fixed on the membership side of the same panel. Defaults true because that is exactly what every existing show did. False does **not** hide the health rows: the office may still record a paper it was handed, and that still clears the flag; the inspection simply stops counting toward `outstanding` and the sweep totals. Asked on the Paperwork step only once a document is required, and stored regardless, so turning a document off and on again does not lose the answer. |
| `137_show_fee_classes.sql` | New table `show_fee_classes` — which classes an automatic `show_fees` charge applies to. **Empty means the whole schedule**, which is the ordinary case and what every existing fee at every existing show gets; rows *narrow* the charge, and an exhibitor who entered none of them owes nothing for that line. Found on a live show: the MNSPHC catalogue carries an "APHA Youth class" rate, an "All Breed WSCA" rate, an "MNSPHC All Breed" rate and two All Day fees split Open/Amateur against Youth — every one an automatic unit, so `billing.charge_lines` billed all five to every exhibitor whatever they had entered, and a single $36 amateur class came out at **$552**. Migration 131 already scopes every automatic charge to "not a club-sanctioned class", but that is one coarse rule and the only one the app had; the top half of a real catalogue is drawn on far finer lines. Deliberately the same shape as `class_sanctioning` (migration 113), which exists because a club approving a *list of classes* is not a property of the show — and a charge applying to a list of classes is not either. It **narrows and never widens**: 131's exclusion still applies on top, so naming a WSCA class in the breed body's own assessment does not make the assessment reach it. Only meaningful on an automatic unit — a reserved fee bills from a booked quantity and a price-list row bills nobody, so neither has a count for this to narrow — enforced in the router rather than by a CHECK, for the reason `min_quantity` and the early rate are: the unit families live in `billing.py`, not in the schema. Server defaults are stated outside the `CREATE TABLE` per 110's rule about `create_all` winning the race, and it won it again here. |
| `136_show_registration_drafts.sql` | New table `show_registration_drafts` — an exhibitor opened a show's registration screen and has not signed up. **Registration's first per-show write is `PUT /signup` at step four**: the details, the memberships and the horses all write `exhibitors` / `exhibitor_registrations` / `horses`, which belong to the person and not to the weekend. So somebody who opened the form, found they needed the horse's Coggins and closed the tab left no trace at all against that show, and `GET /my-shows/` — which finds a show through a `show_entries` row or a class entry — had nothing to list. One row per (show, exhibitor): `started_at`, `last_opened_at`, and nothing else. **Not a `show_entries` shell row**, which is the obvious home and the wrong one: a NULL `registered_at` row is what a secretary creates while adding a late entry by hand, and it is what the desk draws its roster from, so writing one when somebody merely opens a form would fill the office's roster with window-shoppers and inflate the "no back number" and "no entries" counts staff work down. **No progress column**: how far they got is derived on read from `profile_checklist` and their horse count — the same answers the registration screen renders and `PUT /signup` refuses on — and a stored step would go stale the first time they finished their profile from `/profile` instead; what is not derivable, and is the whole row, is that they were looking at *this show*. Disposable in both directions: signing up deletes it in the same transaction as the `show_entries` row that supersedes it, the My Shows card's *Not entering this show* deletes it, and opening the screen again writes it back — somebody returning to the form has changed their mind, and there is nothing worth keeping to argue otherwise. Reads filter to PUBLISHED shows, since that is the status self-registration is open in. Server defaults are stated outside the `CREATE TABLE` per 110's rule about `create_all` winning the race — and it did win it here, the migration logging `relation "show_registration_drafts" already exists, skipping` on the way through. |
| `134_exhibitor_competition_cards.sql` | New table `exhibitor_competition_cards` — one row per (exhibitor, association, division) for a card the exhibitor holds, plus the required-expiry rule on breed memberships that ships with it. **There is no expiry column, deliberately.** APHA gates five divisions on a card rather than on age or membership alone — Amateur, Novice Amateur, Amateur Walk-Trot, Novice Youth and Youth Walk-Trot 11-18 — and every one of them runs 1 January to 31 December and is renewed annually (apha.com/competition/amateurs: "Amateur cards run January 1–December 31 and must be renewed annually"; APHA's own entry form: "ALL APHA AMATEUR, NOVICE AMATEUR, AMATEUR WALK TROT, NOVICE YOUTH AND YOUTH WALK TROT 11-18 CARDS EXPIRE DECEMBER 31ST"). So the row stores `valid_year` and `competition_cards.card_expiry()` derives 31 December of it, the way a horse's age is derived from its foaling date — a date box would accept 2026-06-30, which is not a card APHA issues, and the desk would read the wrong expiry off a screen that looked authoritative. Renewing is raising the year, which is what an annual renewal *is*. A table rather than more columns on `exhibitors`: migration 010's `amateur_card_number` / `amateur_card_expiry` / `amateur_novice_codes` described one association and one card, which is the shape migration 117 had to unpick for membership numbers — somebody legitimately holds an Amateur card and a Novice Amateur card in the same year. Those columns are backfilled into an APHA `AMATEUR` row and **left in place**, for 117's reason. `division` reuses `entries.apha_division`'s vocabulary (migration 115) because a card is what entitles an entry to name that division; `YOUTH` and `YOUTH_WALK_TROT_5_10` are absent because APHA's notice does not name them. **Which associations issue which cards is not in the CHECK** — it is `competition_cards.CARD_DIVISIONS_BY_ASSOCIATION`, which knows only APHA, because guessing AQHA's year-end would produce a card filed here, read at a desk and found out at a gate. Server defaults and constraints are stated outside the `CREATE TABLE`, per 110's rule about `create_all` winning the race. Shipping alongside: **a breed membership may no longer be filed without an expiry date** (`POST`/`PATCH /exhibitors/{id}/registrations`), since NULL has meant *unknown* rather than current since 117 and a breed body's good standing is a condition of showing (APHA RG-030). Enforced in `routers/people.py` rather than by a CHECK, which cannot see `associations.association_type`. A club membership stays optional — several clubs sell one at the gate and plenty run to no fixed date, and refusing the number over the missing date loses the number too. `PATCH /exhibitors/{id}/registrations/{reg_id}` is added by the same change and needed by it: rows predating the rule carry no expiry, and without an edit path the only way to supply one would be to delete the membership and retype the number. |
| `128_registration_wizard.sql` | Add `show_fees.min_quantity`, `show_entries.stall_request` and `exhibitor_horses.relationship_to_owner`. Three questions the exhibitor registration wizard now asks once each, instead of never or on every class. **`min_quantity`** is the floor under a reservable line: a show that bans outside shavings is telling exhibitors to buy bedding here, and "buy some" with no number is a stall bedded with two bags where the show wanted four. It is not the whole floor -- see `backend/reservations.py`: a show that bans outside shavings has already stated the requirement, and almost every show that means it records *that* rather than a number, so a bedding line at such a show carries a floor of one bag whether or not anybody set a minimum. It lives on the fee rather than on `shows` because it is a property of the line being sold. It may only be set on a stall or bedding line: a minimum is a policy the show imposes on everybody, and nobody has to book a camping spot to be allowed to enter. It is a **requirement**, checked against the whole booking rather than line by line -- a range check on the lines that were sent cannot see the one left out, and leaving it out is the easiest way to book none of something, so "at least four bags" would have been met by sending no bags at all. Enforced in the router beside the early-rate guard, and for the same reason: the reservable unit family lives in `billing.py`, not in the schema. **`stall_request`** is kept apart from `registration_notes` because the office reads the two at different moments -- every stall request at once while the chart is being drawn, against "arriving late Friday" at the gate. **`relationship_to_owner`** is how the exhibitor is entitled to show the horse (APHA AM-300.E, YP-015); it was asked on the entry form per class from a list of twenty-five, so somebody entering eight classes on their own horse answered "Self" eight times and could answer differently on the eighth. The column only stores the case the app cannot derive: `horses.owner_exhibitor_id` already answers it for anybody showing their own horse, and `horse_eligibility.effective_relationship` returns Self there without asking. On `exhibitor_horses` rather than on `horses` because two people may show the same horse and their relationships to its owner are different answers; the endpoint upserts the link row, which asserts nothing new since the horse is already on that profile. |
| `127_show_bill_source.sql` | Add `shows.showbill_source` (`generated` default / `uploaded`) and a new `show_documents` table (`SHOWBILL` only for now, one per show). The app has always generated the show bill from the show's own classes, judges and fees, and that stays the default and the recommendation — a generated bill cannot fall out of date with the schedule it describes. What this adds is the option a club with an already-printed, designed show bill was going to use anyway; refusing it never made those shows use the generated bill, it made them e-mail a PDF this app never saw. **The choice and the file are separate facts**: `showbill_source` may only read `uploaded` while a SHOWBILL row exists, enforced in `routers/show_documents.py` because a CHECK cannot see another table, and `DELETE` of the document resets the column in the same transaction. Reads return `effective_source` beside `source` so a renderer can never be handed an empty frame. **An uploaded bill hides nothing** — the generated document is still printed under its own heading (on `/shows/[id]/showbill`, beneath the upload; Show Details did it until the bill came off that page), because that is the fee list `GET /shows/{id}/fees/public` charges from. No staleness check: it would need `updated_at` on classes, fees and judges, and none of them has one, so the upload date is stamped on the page instead. |
...
| `120_class_pattern_posting.sql` | Add `classes.pattern_posted_at` and `classes.pattern_notes`. Every pattern class in the rule book opens the same way — "it is mandatory that the judge post the pattern at least one hour prior to the commencement of the class" (AM-115.B.2, YP-120.B.2, the hunt-seat equitation procedure) — one of the few show-management duties stated as mandatory with a deadline, and the app had nowhere to record it happening. **Neither column is the pattern.** It is posted physically on a board by the gate, and storing an image here would create a second copy that can silently disagree with the one exhibitors actually walked — worse than no copy, because somebody would ride this one. Same reasoning that keeps the show bill generated rather than uploaded. `pattern_notes` is free text because it is usually a reference to a numbered pattern in the rule book and the app has no business supplying a vocabulary for something a judge designs. Nothing is enforced: the app cannot check the hour either, since `classes` carries a date and no start time. |
| `119_show_apha_zone.sql` | Add `shows.apha_zone` (SMALLINT, CHECK 1-14, nullable). Zones surfaced in five separate rules while reading the 2026 rule book and the app had no representation of them at all: class procedure (Zones 12-14 work equitation and horsemanship individually from the gate with no rail work), Green class point thresholds, the `(Zone 12-14)` text sitting inside class names in the loaded catalog because there was nowhere else for it, Zone Shows, and zone year-end awards. Only the first is actionable with the data the app holds, and it is the one a gate steward needs at the rail; the rest need points, show categories or award tracking, and this is the column they will all read. On `shows` rather than on a regional club, because `associations` holds national bodies and there is no regional club table. NULL means *not stated* — nothing derives a default from the venue's state, since a guessed zone is wrong at exactly the shows that sit near a border. |
| `118_entry_attestations.sql` | New `entry_attestations(entry_id, kind, statement, attested_by_user_id, attested_by_name, attested_at)`, unique on `(entry_id, kind)`. **What the entrant declared, recorded rather than verified.** APHA's Novice divisions are gated on points and prize money the app does not hold and never will — AM-205 decides Novice Amateur per category at the time status is applied for, YP-255.A.1 caps Novice Youth fence-work earnings at $750 — and the rule book puts the responsibility on the exhibitor with the burden of proof on whoever protests. `statement` is a stored copy of the exact words rather than a pointer, for the same reason a signed waiver keeps its own text: the wording changes when APHA revises its limits, and a pointer would silently restate what somebody agreed to two seasons ago. It is written by the backend from `rules/apha.py` and never taken from the client, which is the paperwork-verification rule applied to a fact with nothing on file to derive it from. `attested_by_name` is a denormalized snapshot like `coggins_override_audit`'s, so the row survives the user record. `Entry.attestations` is `lazy="selectin"` and the rows are assigned before flush, so the rules engine sees a declaration on an entry that has not been written — a check that queried the database would reject every new Novice entry. Defaults and the CHECK are stated separately from `CREATE TABLE`, as in 114 and 116. |
| `117_exhibitor_registration_expiry.sql` | Add `exhibitor_registrations.expires_at`. The registry every affiliation has read from since 080 held the membership number and never when it runs out, so the desk could show a secretary an APHA number and nothing about whether it was still good — and APHA membership in good standing is a condition of showing (RG-030). Read **beside** the verification status and never folded into it: `status` answers "has anybody here inspected the card?" and `lapsed` answers "is the card good?", the same split health paperwork already makes between a derived standing and an attested one. Judged against `shows.end_date`, never today, for the same reason health is — a card lapsing on the Saturday of a three-day show is precisely what the office needs to chase. NULL means *unknown*, not current, and the desk says so rather than showing green. Backfills from the pre-080 `exhibitors.apha_member_expiry` and creates registry rows outright where a number was only ever recorded the old way; both legacy columns are **left in place**, because dropping the sole home of somebody's membership number is data loss rather than a migration. |
| `116_horse_coat_patterns.sql` | New `horse_patterns` lookup and `horses.pattern_id` — **a coat pattern is not a coat colour.** Tobiano, Overo, Tovero and Sabino had sat in `horse_colors` beside Bay and Buckskin since migration 007, along with six Appaloosa patterns. They are two independent axes and APHA says so outright: the rule book describes the spotting patterns (tobiano, frame overo, sabino, splashed white, dominant white, tovero) in one place and lists its recognized *colours* in another, and a registration certificate reads "Bay Tobiano". One column forced whoever entered the horse to drop half of what the papers said, and the dropped half could not be reported back to the association. The Appaloosa rows move too — they are ApHC patterns, not colours, and leaving them would keep exactly the confusion this removes. Everything is moved **by name** so nothing is invented: the Appaloosa pattern rows are copied out of `horse_colors` with whatever names are actually stored (they contain an en dash), the backfill joins on name, and only then are those rows deleted from the colour list. `horses.color_id` is `ON DELETE SET NULL`, so a row the backfill somehow missed nulls out rather than failing. Defaults are stated separately from `CREATE TABLE` for the reason 114 learned: `create_all` may already have made the table from the model, leaving `id` with no SQL default. |
| `115_apha_walk_trot_divisions.sql` | Add `AMATEUR_WALK_TROT`, `YOUTH_WALK_TROT_11_18` and `YOUTH_WALK_TROT_5_10` to the `entries.apha_division` CHECK. Those are full APHA divisions (AM-300, YP-109, YP-110) with their own class lists, eligibility and year-end awards, most shows run them, and the column had permitted only six values since migration 010 — so a show running Walk-Trot could not record those entries at all. Youth Walk-Trot stays **split by age** because APHA runs 11-18 and 5-10 as separate divisions; collapsing them would have to be undone the moment either is reported on. Two constraints have to go, not one: 010 created the column with an inline CHECK that Postgres auto-named, and 016 then added a second, explicitly-named one saying almost the same thing, so restating only the named one would leave the 010 constraint quietly rejecting every new value. The `DO` block finds them by what they check rather than by a name that was never written down. |
| `114_association_standard_classes.sql` | **One versioned home for every association's class-code catalog.** `apha_standard_classes` and `aqha_standard_classes` were the same table twice, differing only in two columns, with no table at all for ApHC or FQHR — a shape that cannot answer "let an admin upload the new file for association X". New `association_standard_class_versions` keyed on `show_type_id` (a class *code* is the breed body's catalog identifier, the question `class_associations` and `standard_classes` already answer against `show_types` — not an `associations` affiliation), plus `association_class_imports` for the upload audit and the `association_standard_classes` view over the open rows. Type 2 rather than a plain table because the associations reissue their list every year and a show run under a since-retired code still has to render its own program. Both old tables' rows are carried in as the currently-open version (they have been hand-tidied since their original load, so they are preserved rather than re-derived) and the tables are dropped. The migration states its server defaults and its CHECK in separate statements and supplies its own ids on insert, because `create_all` may already have created both tables from the models — leaving an `id` with no `DEFAULT`, since the model's `default=uuid.uuid4` is applied in Python.
...
| `112_show_charge_fee_units.sql` | Add `per_exhibitor` to the `show_fees.unit` CHECK and split `per_judge` into `per_judge_per_horse` / `per_judge_per_exhibitor`, with existing rows migrated to the former. A charge priced `per_horse` or `per_judge` had been storable since migration 060 and had never reached an account: `build_bill` itemises class entries, reservations and futurities, and the only non-class charge it applied was `shows.office_charge_cents` — exactly one, hard-coded on the show row — so a drug fee typed into the fee editor printed on the price list and billed nobody. `per_exhibitor` exists because `office_charge_basis = 'per_back_number'` is a basis for *that one* charge rather than a unit any fee can carry; it is deliberately not `flat`, whose *occurrence* is not derivable (a stall cleanout penalty applies to whoever left a mess). The `per_judge` split is the `per_night` / `per_day` problem again — "per judge" does not say what it multiplies, and the two readings differ by however many horses somebody brought — settled the same way, because `build_bill` reads rate × quantity and never the unit. `per_entry`, `per_class_per_horse` and `percent_of_entry` are left alone and stay unbilled: they are the class-fee vocabulary and `classes.entry_fee_cents` already charges per entry, so billing Step 5's `standard_class` row on top of it would double every class. The new billing half is `billing.charge_lines` / `AUTOMATIC_FEE_UNITS`, rolled up in Financials **apart from** the reservations so the Fees Reserved report's rows keep agreeing with its own total. The whole CHECK is restated rather than amended, as in 106 and 111, and the column gets a `COMMENT` naming the three families. |
| `113_class_sanctioning.sql` | New join table `class_sanctioning(class_id, association_id)` — which classes each sanctioning club actually approves. A club sanction is not a property of the show: NSBA approves a list of classes, and an NSBA-sanctioned show runs plenty of classes NSBA has nothing to do with. The app had nowhere to say that, and both halves of the money were wrong in opposite directions. **NSBA billed every class** — `build_bill` read one show-wide boolean (`show_is_nsba_sanctioned`) and added 6% of the entry fee, $3 minimum, to every line, so an exhibitor who entered eight classes paid eight sanction fees whether or not NSBA approved any of them. **WSCA and MNSPHC billed nothing** — `show_sanctioning.per_class_fee_cents` is set in setup and printed on the public show bill as "$2.00 per class", and no code path had ever read it, so the show published a price and charged nobody. One table fixes both, because both are the same missing fact. Points at `associations` and not `show_types` (clubs are deliberately not show types, migration 080); `class_associations` is a different table answering a different question — the breed association's *class code* for catalog imports. The hard-coded NSBA 6%/$3-min rule is **removed**: every club now bills the flat `per_class_fee_cents` its show row carries, so there is one rule and one place to read it. Backfill designates every class for every club the show already sanctions — for NSBA that is exactly what was being billed, so no open show's bill moves and staff untick what their approval does not cover; for WSCA and MNSPHC it starts billing a fee the show bill had been publishing all along. Written to converge the schema rather than create it, because the backend's startup `create_all` races ahead of it and leaves `id` with no SQL-level default. Bill fields renamed `nsba_sanction_*` -> `sanction_*` throughout, and the class list payload gained `sanctioning_codes` so the show bill can mark which classes carry a fee. |
| `111_per_day_fee_unit.sql` | Add `per_day` to the `show_fees.unit` CHECK and to `RESERVABLE_FEE_UNITS`. Migration 106 added `per_show` for a hook-up sold as one charge for the whole event and 108 folded camping and the hook-up into one line item priced two ways; two ways is one short. Some venues sell the hook-up **by the day**, and a day is not a night — a Friday-to-Sunday show is three days and two nights, so a per-day rate charged against a count of nights under-bills every camper by a day and the reverse over-bills them by one. Settled the same way `per_show` was: a separate unit, so the number the exhibitor types has a noun against it. `build_bill` reads rate × quantity and never the unit, which is exactly why the unit has to be right where the quantity is entered — nothing downstream can recover the difference. Reservable like the other three, so it carries an early-bird rate and appears on the sign-up screen. The whole CHECK is restated rather than amended because Postgres has no "add a value to a CHECK". |
...
**An uploaded bill hides nothing.** `/shows/[id]/showbill` prints the generated
document beneath the upload, under its own heading, because that is the fee
list `GET /shows/{id}/fees/public` charges from.
...
`per_stall`, `per_bag`, `per_night`, `per_day`, `per_show` (`RESERVABLE_FEE_UNITS` in
...
> **Superseded by migration 114.** This table was dropped and its rows carried into
> `association_standard_class_versions`. The section below records what migration 043
> created; refreshing the list is now an admin upload at `/admin/standard-classes`.
...
The 2026 AQHA Class Master Listing is stored as `database/seeds/aqha_standard_classes.csv` after extraction from the official PDF. The loader that filled this table (`scripts/import_aqha_standard_classes.py`) was removed with migration 114 — refreshing the list is now an admin upload at `/admin/standard-classes`, which versions each change rather than overwriting in place.
...
erDiagram
    ...
    exhibitors ||--o{ exhibitor_competition_cards : holds
...
| `association_standard_class_versions` | **Every version of every class code an association has published** (migration 114). Type 2: `effective_date` / `inactive_date`, never updated in place, never deleted. A partial unique index on `(show_type_id, code) WHERE inactive_date IS NULL` allows one open version per code. Replaces `apha_standard_classes` and `aqha_standard_classes`, both dropped. |
| `association_standard_classes` | **View** — the open versions only (`inactive_date IS NULL`). This is the read surface for the class pickers and AQHA validation; read it through `backend/standard_classes.py` rather than querying the versions table, so a retired code cannot reach a picker. Mapped read-only in `models.py` and deliberately removed from `Base.metadata` so `create_all` never tries to make a table over it. |
| `association_class_imports` | One row per applied class-code upload: which association, the filename, who uploaded it, and the added / changed / retired / unchanged counts. `association_standard_class_versions.import_id` points back, so any catalog row can name where it came from. |
...
| `show_bill_imports` | One AI read of an uploaded show bill, and the show a person created from it after review (migration 141): the bill itself (`file_data`), `status` (`pending` while the background read runs), what the model read (`extracted`), the reviewed payload the show was made from (`accepted`), and `show_id`. A `pending` row older than twenty minutes is reported as failed by the reader. See [showbill-import.md](showbill-import.md) |
| `show_companies` | **The business that runs shows** — a club or management firm — and the unit a paid feature is sold to (migration 142). A name and `notes`, which only a GaitDesk admin reads and which hold the billing arrangement while the app takes no payment. Since migration 143 **`owner_user_id`** marks a company that *is* one independent person — named after them, made with their account, following their name until somebody renames it, and deleted with them (`ON DELETE CASCADE`). Names are unique on `lower(btrim(name))` **among organizations only**: two independent people can both be called Sarah Johnson |
| `show_company_members` | The accounts that work for a company, unique on `(company_id, user_id)`. Many to many: a freelance secretary may work for several clubs and gets every feature any of them has |
| `show_company_join_requests` | **Somebody who typed an existing organization's name at sign-up** (migration 143), unique on `(company_id, user_id)`. Not a membership — membership carries the company's paid features, so a GaitDesk admin approves it by adding the member, which deletes the request |
| `show_company_features` | **A paid feature switched on for a company. The row is the switch** — present is on, and turning it off deletes it. `feature` is a key of `show_companies.FEATURES` and carries no CHECK, so adding a feature is one registry entry; a key the registry no longer names switches nothing on. `enabled_by_user_id` / `enabled_at` record who turned it on and when |
...
| `exhibitors` | Exhibitor profile/person records. Since migration 140 a row may also say **who wrote it**: `created_by_user_id` is the staff member who typed this person in at a registration desk (NULL means the row came from an account signing up, or predates 140), and `email` is the contact address the office was given -- a contact detail, never a login. |
| `exhibitor_horses` | Horses an exhibitor may ride beyond ownership |
| `entry_attestations` | What the entrant declared about an entry — kind, the exact words, who and when. Recorded, never verified (migration 118) |
| `exhibitor_registrations` | Exhibitor membership numbers per association, with `expires_at` — NULL means unknown, not current. **Required on a breed association** since migration 134; still optional on a club |
| `exhibitor_competition_cards` | A card the exhibitor holds for one division with one association, for one competition year (migration 134). Stores `valid_year`; the expiry is derived — 31 December of that year — and renewing means raising the year |
...
| `horses` | Horse profile, owner link, optional trainer registry link with free-text fallback, breed/colour/pattern/registration/document links |
| `horse_patterns` | Coat spotting patterns — the second axis of a coat, independent of `horse_colors`. A Paint is a colour AND a pattern (migration 116) |
...
- A competition card's expiry is derived from `exhibitor_competition_cards.valid_year` — 31 December of it, in `backend/competition_cards.py` — and is not stored. Every APHA card runs 1 January to 31 December, so the year is the fact and the date is the rule.
