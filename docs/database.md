...
| `futurities` | A futurity programme within a show (migration 107) — `entry_deadline`, `late_fee_cents`, and `office_fee_member_cents` / `office_fee_nonmember_cents`. Not a `show_fees` row: a futurity prices one class several ways, by entrant category |
...
| `futurity_divisions` | Hi-Point award brackets (Yearling, 2 Year Old), each with a `scoring_method` reusing the side pot vocabulary |
...
| `futurity_entries` | One horse enrolled in one futurity, at one tier, with `is_member` and a stored `entered_at` that decides the late fee |
