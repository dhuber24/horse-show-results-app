...
The hub and every step page read those counts from one helper, `setup/_lib/fetchStepCounts.ts`, so the tab bar on a step page and the checklist on the hub cannot disagree about what is done.
...
- **It is navigation, not a tab widget.** Each tab is a route, so the markup is a `<nav>` of links with `aria-current="step"` on the one you are on — not `role="tablist"` / `role="tab"`, which would promise a screen reader a panel relationship that does not exist. The tab you are on is a `<span>` rather than a link: it goes nowhere, and a control that reloads the page you are already on would flush and discard for nothing.
- **Sticky only from `sm` up.** Pinned to the top of a phone, a four-row tab bar is half the viewport, which is the opposite of getting somebody to the work. The site header is static, so at desktop widths the bar pins to `top: 0` with nothing to collide with.
- **`stepName()` lives in `_wizard/steps.ts`, not beside the tab bar.** `StepLayout` is a server component and `WizardStepper` is a client one; exporting a plain helper from a `'use client'` module and calling it on the server is a *runtime* error ("Attempted to call stepName() from the server"), which neither `tsc` nor ESLint catches — the page renders an error boundary instead. Shared helpers between the two belong in the plain module.

The hub passes `current="hub"`, not `"basic"`: on the hub no step is open, and a tab bar that filled in Basics would tell somebody they are on a step they have not opened. `/admin/shows/new` passes `hubHref={null}` — the show does not exist yet, which is also why every step there is an unlinked preview.

### Skipping A Step

A step nobody is required to complete offers a **skip** at the *top*, above the form (`StepLayout`'s `skip` prop: a `label` and a `note`). It used to sit at the bottom, as one more button in a footer row that already held Back, Setup hub and Next — so the manager who most needed it, the one this step is not their show's business at all, found out only after reading the whole screen for something to fill in. It goes where the Next link goes; what it adds is being told, on the step, that walking past is an answer rather than an omission they will be chased about.

Two rules decide where one appears:

- **Only while the step is empty.** A show that has set a futurity up is not skipping anything, and the control would be offering to walk past work already done. Each page passes `skip={…}` conditionally on its own count.
- **Only where leaving it empty is a supported answer.** Judges, Lodging, Sanctioning, Futurities, Fees and Judge Cards have one. **Basics, the Class Builder and the Show Bill do not** — a show with no classes is not a show (`PUBLISHED` requires one), and telling somebody otherwise in a button is worse than silence. The Show Bill is last and has no next step to skip to.
...
| --- | --- | --- |
| 1. Basics & Staff | `/admin/shows/new`, then `/admin/shows/[id]/edit` | Name, show type, dates, venue — and **everyone who works the show**, via `ShowStaffPanel`: managers (`show_managers`), secretaries (`show_secretaries`), scribes (`show_scribes`), gate stewards (`show_gate_stewards`). Managers and secretaries are picked from `GET /users/by-role`; a secretary can also be inline-created via `POST /users/with-password` (Show Managers may only inline-create `SHOW_SECRETARY` accounts — extended check in `routers/people.py`). Scribes and gate stewards are assigned or invited by email token. Deciding who runs a show is part of setting it up, so there is no separate Show Staff screen; `/admin/shows/[id]/staff` redirects here. The entry deadline (marked optional), the clinic tick and the APHA zone carry **no explanatory text** on the form: the people filling it in run shows and already know what a zone or a clinic means, and what each field changes is written down in `docs/apha.md` and reported on the APHA readiness panel — not restated under the box. |
| 2. Judges | `/admin/shows/[id]/setup/judges` | Reuses `JudgesEditor` — **picks** judges from the `judges` registry (`GET /judges/`) and assigns them with `POST /shows/{id}/judges`. Name, contact details, and association cards are displayed read-only from the registry; show setup cannot edit them. A judge who isn't in the registry yet is added to it (`POST /judges/`) and assigned in one step. Offers **Skip — add judges later** while the panel is empty, whose note names the one thing that waits on it: a club or show fee priced per judge quotes nothing until there is a panel to multiply by. |
| 3. Lodging & Boarding | `/admin/shows/[id]/setup/lodging` | Three structured slots written into `show_fees` with codes `stall` / `shavings` / `camping`, plus a `shows.shavings_ban_outside` policy bool. Each slot also takes an optional **early rate** — a cheaper amount plus a "reserve by" date (`show_fees.early_amount_cents` / `early_deadline`, migration 092). The camping slot is camping *and* the electrical hook-up, with a choice of how it is charged: per night, per day, or one price for the whole show (`per_night` / `per_day` / `per_show`, migrations 108 and 111). A free-text notes field carries "electric included" or similar. **Each slot is its own card under its own heading** — all three used to share one box, separated only by a small grey label above each unit picker, so a note or a minimum typed against one line read as though it applied to the line above it. Extra campsite tiers — dry camping, early arrival, late departure — are not slots here; they go on the full boarding fee schedule at `/admin/shows/[id]/fees/boarding`, which a slot links to when it is holding a unit these slots do not offer. Keeps its Save button, and leaving the step presses it. Offers **Skip — nothing to book here** while no lodging fee exists: a day-haul show that sells no stalls, shavings or camping needs none of these, and the note says so as well as pointing out that what a *class* costs is set in the Class Builder. |
| 4. Class Builder | `/admin/shows/[id]/classes` | Build the class schedule — the single-screen Class Builder documented below, or the per-association importers. The grid's own columns and rows (disciplines and divisions) are added from a panel over it; they were two steps in front of it until they were collapsed in, because neither is read anywhere else in setup and a show is no closer to having a schedule for having named either. Longest job in setting a show up, which is exactly why it is a step in the flow rather than an errand to remember from the dashboard. Route unchanged, so per-class deep links still work. It used to carry two yellow notices at the top, one linking to Sanctioned Classes and one to Judging Cards; both are steps of their own now (5 and 8), which the stepper already shows and the Next link already walks into. A banner pointing at the next step is a second door, and these sat above the wizard somebody came to the page to use. |
| 5. Sanctioning | `/admin/shows/[id]/setup/sanctioning` | **All three questions a club sanction asks, on one screen.** Pick zero or more clubs from `sanctioned_associations` (NSBA, WSCA, MNSPHC …); set what each charges **and how** — an amount plus a unit, per class entered / per exhibitor / per horse / per judge per horse / per judge per exhibitor (`show_sanctioning.fee_unit`, migration 133); tick the classes each one approves. Each club's class list toggles between **All** and **Ticked**, the text filter narrowing whichever view is showing: ticking a 172-row schedule and checking the twenty-one rows you ticked are different jobs, and only the second can be read down against a paper show bill. Wraps `PUT /shows/{id}/sanctioning` (which replaces the full set — membership and rates together) then `PUT /shows/{id}/classes/sanctioning/{association_id}` per club whose class list changed; that order matters, because designating a class for a club the show does not carry is a 404. One Save for the step, and leaving the step saves it. The "+ Request new sanctioned club" link expands the request form on demand (`POST /sanctioned-association-requests`) — admin reviews via `POST /sanctioned-association-requests/{id}/review`. Optional, and it says so: **Skip — no club sanctioning**. This was Step 3, before any classes existed, with its fee in a box on the fees step and its class designation on a screen hanging off the class list — three screens for one decision, none of which charges anybody on its own, visited in an order nothing told you. |
| 6. Futurities | `/admin/shows/[id]/futurities` | Optional; most shows run none. **+ Add futurity** captures the whole published entry form — deadline (to the minute), late fee, office fee by membership, the entry categories and what qualifies for each, an optional club membership for sale, which classes belong to the programme, the award / rules / refund notices, and the release entrants sign. After the Class Builder on purpose: a futurity is defined by which classes belong to it, and there is nothing to pick from until the schedule exists. Route unchanged, so the dashboard tile still reaches it. Offers **Skip — no futurity at this show** while the show has none. |
| 7. Show Fees | `/admin/shows/[id]/setup/fees` | **After Sanctioning and Futurities**, so it closes with an **Already priced in earlier steps** panel (`setup/fees/EarlierFees.tsx`): each club's rate and unit with its approved-class count, and each futurity's categories, office fee and late fee — read-only, with a link back to the step that sets them, so this is the one screen showing the whole of what an exhibitor is billed. That panel sits **below** the Class Fees box: it is reference, and when it opened the step it put a screen of figures the manager cannot edit here between them and the box they came to fill in. The step used to come before the Class Builder, when the class-scope picker on a fee was always empty for a show set up in order. Everything that is a class fee lives in one **Class Fees** box (`components/ShowChargesEditor.tsx`) — no class fee is set up anywhere else, and after this change nothing else is on the step at all. There is no dedicated "Standard class fee" slot any more; a show that priced one keeps the row, now just an ordinary, renamable line in the list. **+ Add a fee** opens a blank row **above the list**, and the row it creates lands at the top too (`sort_order` below every existing row) — a new fee appearing at the foot of a dozen others is one nobody sees they just made. Every row carries a **Note for the show bill**, printed under that fee on the published bill: it is where a scope or an arithmetic the row cannot express itself gets written down. What a row actually charges ("$45 × 4 judges = $180 for each horse they enter") is the unit picker's tooltip, not a line of prose repeated under every fee. There are **no quick-add presets** any more — six pre-named chips (office fees split by horse and by judge, the assessment, an all-day pass, a jackpot) read as fees the show already had, and two were the same unit under two names. A side pot's buy-in is set on the side pot itself, over only the classes bundled into it — not here, where a *per class* row charges every entry. A fee **quoted per class** (*per class*, *per judge, per class*) carries *Choose which classes use the fee* — every class ticked under **Select all** by default, unticked to narrow it; see "An Automatic Charge May Name Its Classes" below. Per-horse and per-exhibitor charges show no class list. Named charges are priced per class, per exhibitor, per horse, per judge per horse, per judge per exhibitor, or per judge per class (migrations 112, 125; the units are `per_entry` and `per_judge_per_entry` internally) — every one of those six counts only the breed association's own classes, and the two per-class ones also skip futurity classes, unconditionally, the same way the assessment always did; there is nothing to opt into or check. These automatic units are the only fee lines that actually bill, via `billing.charge_lines`; matched by *unit* rather than by code, because a manager names them, and charged to everyone who has entered a class rather than booked like a stall. The office charge is simply one of these rows (migration 132). The same editor is what `/admin/shows/[id]/fees/entry` renders, folded into that screen's single outer box alongside the **per-class pricing** table — the latter can bulk-fill a default price across every still-unpriced breed-association class in one action, skipping club-sanctioned, qualification-only and futurity classes on purpose. **The unit vocabulary says "class", not "entry"**: `per_entry` renders as *per class* and `percent_of_entry` as *% of class fee* (`UNIT_LABEL` in `lib/fee-units.ts`) — a show fee is charged against a class, which is the word on every show bill, and "per entry" beside "per judge, per class" was two names for one thing. The column values are untouched. **Two things left this step**: the *Club Sanctioned Fees* box, to Step 5 where the clubs are picked and their classes designated; and the *Save & continue to Classes* button, which once the sanctioning amounts moved wrote nothing at all — every class fee above it already saves a row at a time — so it was a Save button that saved nothing beside a Next link that goes to the same place. A **futurity slot used to sit here and does not any more** (migration 107): one amount cannot say that the same class costs $75, $100 or $150 depending on the entrant's category. What stays is one line saying where sanctioning and futurities live, and the offer to delete a leftover `futurity` fee row, which is a `show_fees` row on this show and so is genuinely this step's business — a show carrying both bills its futurity entrants twice. Offers **Skip — no show fees** while the box is empty. *Per class* charges once per class entered in its ticked classes, on top of any price on the class row itself, so a class is priced in one place or the other; *per class, per horse* is withdrawn, because two horses in one class are already two entries. |
| 8. Judge Cards | `/admin/shows/[id]/classes/judging` | Which card each scored class is marked on (`classes.judging_system_id`, migration 122), and what each card asks the judge for. A step rather than a notice at the top of the Class Builder: it is a per-class designation made once the schedule exists, exactly like club sanctioning and a futurity's class list, and this screen is the only place the card shapes are explained — which a secretary choosing between three of them needs in front of them. Only `pattern` and `time` classes appear; a rail class is placed, not scored. Each pick saves immediately, so there is nothing for the step's autosave to flush. Offers **Skip — score by total**, because leaving a class unset is a supported answer: it then scores exactly as it did before cards existed. Route unchanged, the same way Step 1 stays on `/edit`. |
| 9. Show Bill | `/admin/shows/[id]/setup/showbill` | Which show bill exhibitors see. **Showbill generated by GaitDesk** is the default — drawn from the judges, classes, fees and policies the eight steps before it set, so it updates itself when any of them change — and carries **Preview the show bill as exhibitors will see it** beside it. **Our own show bill, uploaded** publishes a PDF or image the show supplies instead (migration 127), with an **Upload** (or **Replace**) button beside it and, once a file is on record, one line under the option giving its name, size, upload date, whether it is published, and View / Remove. That replaced a whole section below the choice — a heading, a paragraph of accepted formats (now the button's tooltip), a file input and a file card — for what is one button and one line. Uploading and publishing are still two separate presses: a manager comparing their club's PDF against the generated bill can put it on file and look at it without every exhibitor's Show Bill button changing underneath them. The radio for the uploaded bill is disabled until a file exists (`PUT /shows/{id}/showbill-source` 422s the same case anyway), and removing the file puts the show back on the generated bill in the same transaction. Last in the flow because the show bill is what every step before it adds up to. |

Sanctioning associations are distinct from breed `show_types` — see `docs/database.md`. The breed `show_type` is set once on the show row at creation and drives breed-specific rules; sanctioning is a per-show overlay that adds points eligibility (and an optional fee) without changing the show's primary type.

That fee is charged **only on the classes the club actually approves** (`class_sanctioning`, migration 113). All three facts — the club, its rate, and its class list — are now set on Step 5, in that order, because none of the three charges anybody on its own: a $3 per-class fee against zero approved classes is indistinguishable from a working sanction, and that used to be invisible across three screens. Each club's row states the arithmetic as it is set ("$3.00 × 21 approved classes = $63.00 on an entry in every one", or "$45.00 × 4 judges = $180.00 for each horse they enter in its 21 approved classes") and says outright when it has no fee set. A dual-sanctioned class carries both clubs' fees.

**The rate has a unit** (migration 133), chosen from the same list Step 7 offers on the show's own class fees: per class entered (`per_entry`, the default and what every club charged before this), per exhibitor, per horse, or either of the last two multiplied by the judge panel. `per_judge_per_entry` is not on the list — that is the breed body's own per-entry assessment and lives in the show's fee catalog. Whichever is picked, the count is taken over that club's approved classes only: a `per_horse` club charges for the horses that entered *its* classes, and an exhibitor who entered none of them owes it nothing. The two units that multiply by the panel need judges assigned, and the step says so outright when there are none rather than letting a manager discover it on somebody's bill. On the bill itself, a `per_entry` club's money rides on each class line as it always has, and the others appear as their own itemised line with the arithmetic spelled out.
...
`ClassWizardClient` (`/admin/shows/[id]/classes`, every show type) is **one screen**: the grid, the things it is drawn from, and the schedule it produces. Its UI contract:

- **There is no step, and no panel, for disciplines and divisions — they are the picker's own columns and rows.** It ran as three parts behind a *Step N of 3* progress bar — pick the riding styles and save, pick the brackets and save, and only then reach the grid that crosses the two. A discipline is a **column** of that grid and a division is a **row** of it, neither is read anywhere else in setup, and a show is no closer to having a schedule for having named either: three screens to answer one question. The **Quick Class Picker** draws every column and row the show could use — its own, merged with its show type's standard library — so a show with nothing set up at all opens on a full grid (26 × 18 on APHA) and the first thing anybody does is click the class they want.
- **A cell creates whatever it needs.** Clicking `(Division × Discipline)` where the show has no row for either creates the discipline, the division and the class, in that order, on the one click. `axisIdFor` carries a `known` map across the whole drain, so sweeping six cells down a column the show has never used creates that column **once**; ids come out of the bulk create's own response rather than a re-read, and each axis the drain opened is re-read once at the end. Names go to `/disciplines/bulk` and `/divisions/bulk` rather than the single create, because bulk is what infers a discipline's score type from its name — and a class inherits its score type from its discipline, so guessing it on the client would be guessing how every class in that column is judged. A new division is wired to every discipline the show has; a new discipline needs no matching backfill, since `POST /shows/{id}/classes` upserts the `(discipline, division)` membership as it creates the class.
- **Both axes are alphabetical**, as are the by-name form's two pickers. `sort_order` means nothing for a library name the show has not used, and a thirty-name axis is read by hunting for a name.
- **The × on a heading appears in one case only** (`AxisRemove`): a show row with no classes whose name the standard library does not carry. A library name is a column whether or not the show has a row for it, so removing that row would change nothing on screen; a row with classes is refused by the endpoint, and its classes are what somebody actually means to delete. What is left is a name somebody typed themselves whose classes have since gone — an extra column nothing else can clear. **How many classes use a row is counted from the loaded class list**, not read off the axis endpoint's `class_count`, which a class delete does not refresh.
- **Three buttons stacked at the top, and the picker folds.** *+ Add Class*, *+ Add a Grand & Reserve Class*, then *Quick Class Picker* — a column rather than a row, in the order somebody reaches for them: naming a class is the deliberate one-off, and the picker is the bulk tool underneath. The stack is an `inline-flex` column so all three match the widest label instead of running ragged. **Which of the picker and the class list is open depends on whether the show has a schedule yet**: a show with no classes opens on the picker, since there is nothing to read and the picker is what builds one; a show with classes opens on the class list, with the picker folded away rather than pushing a hundred rows below the fold. Read once on mount, so folding either is not overruled by the next class added. `DualScrollBox` caps the open picker at 70vh. **Both forms open above the picker**, not below it: under a grid capped at 70vh a form was off the bottom of the screen, and the button read as having done nothing.
- **The step opens with three bullets, not a paragraph.** What the picker is for, that the right day is chosen when adding classes, and that a created class is reordered by dragging. *Must Qualify* had a fourth bullet here and lost it — it is explained where it is used, on the line above the class list, which also says a class with entries cannot be deleted. It replaced a block of prose that explained the grid's geometry — which the grid itself shows — and named the standard library, which nobody has to know to use it. Naming it was the only thing `standardLibraryLabel` was ever for, so the page no longer computes one — the library itself is still fetched and still merged into the axes.
- **No footer of its own.** Everything on the screen saves on the press that makes it, and the step's Back / Next is `StepLayout`'s, directly below. The bar that used to be here walked the three inner parts; with one screen its only remaining button went where the footer underneath it already goes.
- **The picker sits above the schedule.** Clicking a `(Division × Discipline)` cell creates the class immediately. The grid scrolls inside its own box (capped at 70vh) so the **discipline row and the division column stay pinned** while it scrolls — `position: sticky` only pins to the nearest scrolling ancestor, and a box that scrolls sideways already is one — and a **second horizontal scrollbar runs along its top edge**, kept in step with the box's own (`DualScrollBox`), because the bottom one is below the fold on any grid wide enough to need it. Cells use separate borders: a collapsed border belongs to the table and scrolls away from a sticky cell.
- **+ Add Class**, with an *+ Add a Grand & Reserve Class* shortcut below it, both in the top button stack. The picker names every class "{Discipline} {Division}", which no championship class is, so there was nowhere to add one. Its two pickers offer the same alphabetical options the grid's axes do, library names included, and `resolveAxisPair` creates whatever rows the save needs. They also each end in **+ Add a new …**, which reveals a name box beside the select; that name is held **locally** and only written on the save that uses it, so a form somebody filled in and closed leaves no column behind. The select never takes `__add__` as its value — it snaps back to the last real option rather than sitting on one that names nothing — and a locally-typed name is merged by key once it becomes real, or the list would carry it twice. The form takes discipline, division, day, the name, **where in the day it runs** (after any class that day — a Grand & Reserve class belongs straight after the classes it calls back from, and dragging it there down a hundred-row list is the job nobody finishes) and **Must qualify**. The shortcut opens on Halter with the name started, Must qualify ticked, and the class placed after the last class that day in the same cell. A qualifying class does not mark its grid cell ✓, so adding one never hides the ordinary class for that cell. **Saving closes the form**, opens the class list, clears any search that would have hidden the new row and scrolls to it — the point of the form is the class it produces, and it used to stay open over a folded list reporting the save in a line of text.
- **One class of a name per day.** The Add button is disabled, with the clashing class named, when that day already runs a class of that name — pressing the Grand & Reserve shortcut twice suggests the same name twice, and the grid's own cell lock never covered the by-name form. The screen is the affordance: `POST /shows/{id}/classes` refuses it (409) whoever asks. Keyed on the name rather than the grid cell, so "Grand & Reserve Amateur Mares" and "…Geldings" can share a cell and a day — see the Sharp Edge in `CLAUDE.md`.
- **Must qualify is a checkbox, and unticked is open entry.** The line above the list reads *"Select Must qualify for classes that exhibitors must qualify for; i.e. Grand & Reserve classes. A class cannot be deleted if there are current entries in it."* Each row of the *Classes added* list carries it (saved optimistically, per row). It replaced a *by qualification* badge beside an *Open entry* / *By qualification* toggle link, two labels for one yes/no.
- **The whole class row is the drag handle, so tap and hold anywhere on it starts a reorder.** It used to be the ⠿ alone — about ten pixels of target on the phone a desk is actually worked from, which made the long-press drag `@hello-pangea/dnd` already supports effectively unreachable. The ⠿ stays as the visual cue and the row carries `touch-action: none` so a press that is becoming a drag does not scroll the page instead. Safe with a checkbox and a Delete button inside it: the library refuses to start a drag whose event began on an interactive element, so those still do what they say.
- **The class list is searchable and sweepable.** A search box filters it on name, class number and day together (every term must match), and each row carries a tick box: *Tick all N shown*, then one **Delete selected** with inline confirmation, posting to `POST /shows/{id}/classes/bulk-delete`. A show runs to hundreds of classes, and clearing a mis-built stretch of one was a delete and a whole-show renumber per row. **A class with entries cannot be ticked** — its box is disabled and its entry count sits beside the name — and the endpoint refuses the whole sweep if one slips through. Filtering turns dragging off, with a line saying so: a drop index into a list with rows filtered out of it moves the wrong classes.
- **Leaving the step waits for the cells still saving.** The finish button used to be disabled while the create queue drained; there is no finish button now, so the step registers a `StepAutosave` flush that waits for the queue to empty before Back / Next navigates (capped, so a request that never returns cannot trap somebody on the step). The flush waits on the queue rather than on "the drain promise": a click can legitimately start a *new* drain while the previous one is finishing its closing refresh, so there is no one promise to hold — see the Sharp Edge in `CLAUDE.md`.
- **Changing the show's dates strands the classes already built, and the step says so at the top.** Moving a show is an ordinary thing to do mid-setup and `PATCH /shows/{id}` takes it without a word: a class keeps the day it was built on. Two things then hid the result. The grid draws a column per **current** show day, so a stranded class marks no cell; and `PATCH /classes/{id}` checks `class_date` against the show, so *every* edit to one is refused with "Class date must be between …" — including edits that have nothing to do with the date. The schedule simply looked short, and the classes could be neither found nor fixed. So the step opens with a warning naming each of them, its day, and a **Move to…** picker of the show's current days; the show days a class of that name already runs on are offered disabled, because `update_class` does not re-check the one-class-per-name-per-day rule and moving one there would slip a duplicate past it from the side. A class that genuinely no longer belongs is deleted from the class list below instead. **The strays are ticked and swept**, the same shape as the class list's own bulk delete: a tick box per row, *Tick all N*, and one day picker that moves every ticked class together — the sweep sitting **above** the rows, beside the *Tick all* that fills it, rather than under them, because it is what somebody does *to* the list and a list of forty strays put it off the bottom of the warning — a show moved a week strands its whole schedule, and re-dating forty classes a row at a time is the job nobody finishes. Two rules keep the sweep honest against the one-name-per-day rule the endpoint does not re-check. A day any ticked class clashes with is **offered disabled**, counting how many; and ticking **two classes of the same name** disables the sweep outright with a line saying which — no day can take both, so every option dead would say "nothing works" without saying why. There is no bulk endpoint and so no transaction: the sweep PATCHes one at a time, **stops at the first refusal, and un-ticks only what landed**, so the ticks left behind are exactly what still has to move and pressing again is the retry. **The date change is saved rather than refused**: the show's dates are the fact that changed, and a manager who has just moved a show is not helped by a form that will not accept the move until they have re-dated forty classes first. **Nothing moves on its own either** — a shortened show has no honest guess at where a class on the dropped day belongs, and a silent shift would re-date a schedule somebody had deliberately laid out.
- **Adding a class renumbers the whole show 1..N** (`_renumber_classes`), the grid and the by-name form alike. A show imported with printed numbers such as "2-3" or a futurity's "A" loses them on the first add — existing behaviour, worth knowing before adding to a seeded schedule.
...
`/admin/shows/[id]/desk` is where the show office works on an exhibitor. Back numbers, class entries, and paperwork check-in were three screens (`/entries`, `/back-numbers`, `/check-in`) and are one conversation at the counter — somebody walks up, gets a number, says what they are riding and on what, and hands over their papers. Splitting that across three pages meant finding the same person three times, and no page could tell you what was still outstanding on the other two. All three routes now redirect here.

- **Two views, one screen.** *By exhibitor* is the desk flow: a searchable roster on the left with filter chips (no back number, paperwork to check, health flags, no classes yet), and the selected person's whole standing on the right. **The roster lists people, not exhibitor records**: a back number lives on the exhibitor record, one per show, so somebody on the desk twice with two numbers is two records sharing a name — usually one left behind when an old account was deleted. `DeskClient` groups by normalised name, the row shows every number (`#24 · #6`), and the panel stacks each record's own panel under a note saying they are separate records. Grouped for display only, never merged — a name is not proof of identity — so the office judges and removes the one that should not be there. *By class* is the program listing — who is entered in what, grouped by show day, with owner/sire/dam. Clicking a name there jumps back to that person's panel.
- **Entries can be added from either view**, because filling a class is its own job — a secretary working down a short class calling for more riders is thinking about the class, not about each rider's account. `AddEntryForm` is one component with one side pinned: pass the exhibitor and it offers a class picker, pass the class and it offers an exhibitor picker. Writing it twice would have meant two copies of the SPB guard, the Novice declaration, and the horse lookup. **It asks for a class and a horse and nothing else**: the APHA division is read off the class (`apha_division` on the desk's class payload, from `rules.apha.division_for_class`) and stated under the picker, and the relationship to the owner is derived from ownership or the horse link — `POST .../entries` fills both from the same sources, so there is nothing a picker could add but a wrong answer. An expanded class shows the form behind a **+ Add an exhibitor to this class** toggle (one open at a time — with *Expand all* on a 21-class show a form per class is a wall of dropdowns), and it stays open after each save so a queue of riders goes in one after another. Closed classes offer a note instead of the form.
...
- **A class in a side pot asks for the buy-in in the same press, and that one is required.** The pot bundles the class, so entering it *is* buying in — see *A Side Pot's Classes Come With The Buy-In* below. The form lists the covering pots with their buy-ins, ticks the one where there is only one (nothing to decide), and disables Enter class until a pot is ticked when there are several. A pot the exhibitor is already in is listed ticked and disabled, which is the answer to "why is this class not asking me for another $50". `side_pot_ids` goes on the entry request so the buy-in and the class commit together.
- **One money line per exhibitor, not two.** Billed, paid, and owing sit on the panel's summary row with a *Record a payment* link straight through to that exhibitor's own account — `/financials/exhibitors?exhibitor=<id>`, which opens their row, switches the filter off *Owing* so a settled account is still shown, and scrolls to it. That row carries a **← {name}'s registration** link back to `/admin/shows/[id]/desk?exhibitor=<id>`, which reopens their panel here: taking money and fixing what it is for are one conversation at the counter, and before this each hop meant finding the same person in a second roster. A second Account block lower down the panel restated the same three figures from the same `build_account` call — two places to read one number, and the reason to scroll for it was a link that belongs next to the figures anyway. The caveat it carried, that side pot buy-ins are not part of that balance, now rides on the side pot count in the same summary row — the Side pots section that held it for a while has gone (below).
- **There is no Side pots section on the panel.** It listed every pot with an *Add to pot* / *In the pot* toggle, and that stopped making sense the moment entering a class an open pot bundles became the buy-in (*A Side Pot's Classes Come With The Buy-In*): the toggle was a second route to what the entry form now does as part of the entry, and the more misleading one, since it could put somebody in a pot whose classes they had never entered. What remains is the count on the summary row, whose `title` names the pots, plus a buy-in line per pot in the Classes section — the money the removed section used to print, now quoted off the bill that charges it (*The Buy-In Is On The Bill*, below) rather than summed in the browser. Adding or removing a buy-in on its own is the pot's own **Side Pot Entries** screen, where the pot is the subject. The desk *payload* is unchanged and must stay so: `side_pots` (with `class_ids`) and each exhibitor's `side_pot_ids` are what the entry form reads to know which classes oblige a buy-in and which pots they are already in. Scratching the last class an exhibitor holds in a pot **releases the buy-in** at both deletion doors (*Scratching Gives The Buy-In Back*, below), so the section's one remaining job — undoing a buy-in whose classes are gone — is done by the scratch itself.
- **The paperwork warnings are jump links, not counts.** *3 paperwork checks outstanding* opens the Paperwork section and scrolls to the first check still owed, naming it in the tooltip; each health line (*Smokin Hot Roxy: No Coggins on file*) goes to that horse's own row, because "no Coggins on file" across three horses is three different pieces of work. The target is ringed for two seconds — on a laptop at the counter, unfolding a long section is not the same as showing somebody the row. Anchors are derived from the check's subject, not its position, since the desk reloads after every save.
...
- **`POST /shows/{id}/desk/exhibitors` is the one exception**, and it only creates the roster row those endpoints assume. A back number lives on `show_entries` and a side pot entry points at it, so before this the desk could not give a walk-up a number or put them in a pot without first inventing a class entry for them. `registered_at` stays NULL — this is the shell row, not a sign-up, and the exhibitor still has to sign up before they can self-register for classes. Idempotent. `DELETE /shows/{id}/desk/exhibitors/{id}` (**Remove from this show**) takes a registration off the show however it got there — a self-registration included — along with every class entry, reservation, futurity enrollment and side pot buy-in it booked; see *Cancelling A Registration*.
- **`POST /shows/{id}/desk/exhibitors/new` creates the exhibitor record itself** (migration 140), for somebody the app has never seen. A first and last name is the whole requirement, because that is what is on the paper entry blank being handed across; email and phone are optional, and **no `users` row is created** — a login belongs to the person who will sign in to it, and inventing an address and a password for somebody who has not asked is not the office's business. `created_by_user_id` records the staff member who typed it in, which is what tells this record apart from the accountless seed data the name pickers exclude. **Deliberately not idempotent**, unlike adding an existing exhibitor: two people at one show really can be called Sarah Johnson. The horse is a separate press against `POST /shows/{id}/exhibitors/{id}/horses`, offered once the person exists, because most walk-ups already have theirs on file.
- **Two records of one person can be joined** — `POST /shows/{id}/desk/exhibitors/{id}/merge`, over `backend/exhibitor_merge.py`, the same implementation the admin's `POST /exhibitors/{id}/merge` uses. The desk has grouped same-name records under one roster entry since it was built; this is what finally lets staff act on it. At least one of the two must be on this show's roster, and the record in the path is the one that survives. **Two records that both hold accounts are refused at the desk** (`TWO_ACCOUNTS`) and allowed for an admin, because deciding which of somebody's two logins is the real one is not a registration-counter question. `merge_candidates` ranks suggestions and says why: a matching email the office wrote down is the best reason this app can offer, a shared name is no evidence at all, and both are only ever a reason to ask.
...
- **Waiver signatures are the one value the backend does not derive.** There is nothing to read a signature off, so `signed_name` is accepted from the client — and staff typing a name off a paper blank is the point. `on_paper` is still set by the endpoint rather than the caller, so the two routes stay honest about which one a row came through. Editing a waiver's text leaves existing signatures alone; deleting the waiver takes them with it. **A futurity's release is one tick at the desk** — *Signed release on file* — which sends no name; the endpoint records the exhibitor's own. Only where the waiver carries `futurity_id`: the show's own entry blank and releases still 422 without a name, because that is where a guardian signing for a minor matters. A release the exhibitor signed in the app shows ticked and locked, with Undo beside it.
...
- `POST /shows/{id}/desk/exhibitors/{exhibitor_id}/cancel` — the office's. No window: someone whose truck breaks down on the Friday still has to come off the stall chart. Distinct from `DELETE /desk/exhibitors/{id}` — **Remove from this show** — which deletes the registration outright rather than marking it: for a duplicate, a test sign-up, or somebody the office would rather not carry as a cancelled row. It used to refuse anybody who had signed themselves up. It drops what the registration booked, as a cancellation does, and then the row, and it is **refused while any payment is recorded** (`PAYMENTS_RECORDED`) — the row cascades to `show_payments` and a refund is never a deletion, so the desk disables the button with a tooltip pointing at the cancel. Both paths share `cancellations.registration_blocker`.
...
- **Refused** once a placing exists, or the exhibitor is in a side pot that has been settled — paid out or not (`RESULTS_RECORDED` / `SIDE_POT_SETTLED`). At that point the exhibitor did not cancel, they competed, and taking an entry out of a settled pot would change a pool whose payouts are already written.
...
**One screen for the whole registration.** `/shows/[id]/register` carries every step, as collapsible boxes (`RegistrationSection`) under a stepper (`RegistrationStepper`) and over the running bill, with the cancel control below it. Two of the six are conditional — memberships at a show with no affiliation, futurities at a show that runs none — so the stepper is built from data rather than from a fixed list:

| Step | Holds | Backed by |
| --- | --- | --- |
| 1. Your details | The `details` half of the checklist and the personal-details form | `GET /shows/{id}/register/preview` → `profile`, `PATCH /exhibitors/{id}` |
| 2. Your association memberships | The exhibitor's own membership numbers, added and edited in place. **Only at a show with a breed or club affiliation**, and it blocks nothing — see below | `preview` → `registrations` + the `memberships` checklist row, `POST/PATCH/DELETE /exhibitors/{id}/registrations` |
| 3. Your horses | Each horse with its registration warnings, its health warnings, its relationship-to-owner picker, and **Remove** — refused while the horse is entered in a class here, which the row says instead of offering the button | `preview` → `horses`, `PUT .../register/horses/{id}/relationship`, `DELETE /exhibitors/{id}/created-horses/{hid}` or `./linked-horses/{hid}` |
| 4. Stalls, shavings & camping | `ReservationFields` — the fee groups, the stabling request, arrival/departure dates, notes to the office | `GET/PUT /shows/{id}/register/signup` |
...
**Memberships are asked for, in the flow, and still refuse nobody.** They used to be a line at the bottom of step one — the checklist hint and an *Add my numbers →* link to `/profile?tab=memberships`, which is a different screen with no way back to the show being registered for. That is the trip step one's own in-place form exists to avoid, so the numbers are now a step of their own between the person and their horses: `MembershipsStep` renders the shared `ExhibitorRegistrations` editor against `preview.registrations`, and every write goes to the same `POST/PATCH/DELETE /exhibitors/{id}/registrations` the profile screen uses. Three things about it are deliberate. It is **the person's own card**, which is why it sits before the horses — a *horse's* papers with the same association are a different fact and are checked on the horses step. It **gates nothing**: `exhibitor_profile.py` marks the membership row advisory, `PUT /signup` does not refuse over it, and the horses step is still unlocked by the details alone — a number typed here is a claim the desk verifies against a card, and one can be bought at the counter. And the wizard **never opens on it**: `initialStep` runs details → horses → stalls, because opening on an optional step tells somebody it is required. Competition cards are linked, not asked for — a card says which division you may enter and nothing here is refused over one, so a second list under this one would make an optional step look like two.

**A registration somebody abandons is bookmarked on My Shows** (migration 136). The first per-show write in this flow is `PUT /signup` at step four: the details and the memberships and the horses all belong to the exhibitor's own profile, so somebody who opened the screen, found they needed the horse's Coggins, and closed the tab left nothing at all pointing at that show — and `GET /my-shows/` finds a show through a `show_entries` row or a class entry, so it had nothing to list. `show_registration_drafts` is one row per (show, exhibitor), written by `POST /shows/{id}/register/draft`, which `RegisterShowForm` fires when it opens. See **Unfinished Registrations** below.

**Futurities are asked before the classes**, which is not where they started. A futurity enrollment adds a line to the bill (`billing.futurity_lines`) and its classes are ordinary `classes` entered in the step below it — so asking afterwards meant somebody read a running total under the class picker that was about to change, and a $150-per-class futurity is not a small change. In this order the total under the last step is the whole of what the show will collect.

**Adding a horse comes back to the step it left.** Step 3's *Add a horse* links to `/profile/horses/new?next=/shows/{id}/register?step=horses`; `NewHorseWizard` pushes that destination on create, on link **and on cancel** (somebody who changes their mind still wants their registration back), through `safeNextPath` because a `?next=` is a URL a stranger composes. `RegisterShowForm` reads `?step=` and opens that box, validated against the steps actually on offer so a stale or hand-typed value falls back to the derived one. Before this, a six-step detour onto another route dropped people on their profile, and finding the show again and re-opening the step is most of the reason a registration gets abandoned half-done.

**The add-a-horse wizard keeps a draft, and it is not a server autosave.** The horse row does not exist until the last step — creating one early to `PATCH` as somebody types would leave half-finished horses on profiles and, in ride mode, fire an owner-approval request for a horse nobody finished adding. So the answers live in `localStorage`, per browser and per exhibitor, restored on mount and cleared the moment the wizard succeeds or is cancelled. The restore is **announced** with a *Start over*, because a form mysteriously pre-filled with last week's horse is worse than an empty one. **Health documents are deliberately not in the draft**: a staged `File` cannot be serialised, and a restored list with no bytes behind it would be a lie the Add button discovers.

**The Registrations step opens with the registry the breeds imply.** `lib/breed-associations.ts` matches a ticked breed to the breed-type association whose name begins with it — "American Paint Horse" to APHA, "Appaloosa" to the Appaloosa Horse Club — and preselects the first one with no number against it yet, moving to the next after each Add. The prefix test is strict on purpose: nothing reaches Foundation Quarter Horse Registry, which is a different registry for the same breed and not something to guess at. It is an **affordance and never a stored inference** — nothing is written, the picker can be changed, and the number is still typed by hand, which is what makes matching on a name acceptable at all where the repo otherwise refuses to guess. Clubs are never suggested: what a horse *is* says nothing about WSCA.

**Health Records says a copy of the certificate is required.** A record here is the document, not a note that one exists — the show office reads the expiry date off the file to work out whether a horse is covered for the weekend, and the desk checks the paper itself against the animal in the trailer. Dates with no scan behind them would clear a health flag on nobody's authority, so the step says so above the form rather than letting somebody discover it at the Add button.
...
**A minimum is only offered on stalls and bedding.** Camping is not something a show requires of everybody in order to let them enter, so the Lodging step no longer shows the box against it and `POST/PATCH /shows/{id}/fees` refuses a minimum on a camping unit (`per_night` / `per_day` / `per_show`). That is narrower than the early rate sitting beside it, which applies wherever a quantity is booked.
...
An exhibitor abandons a registration in two different places, and until migration 136 only one of them left anything behind.

**Signed up, no classes entered.** A `show_entries` row exists, so the show has always appeared on My Shows — as an ordinary card reading "0 classes", which at a show with nothing but halter on the Sunday says the same thing as a finished registration. `hasNoClassesYet()` in [frontend/lib/my-shows.ts](../frontend/lib/my-shows.ts) is `registered_at` set, `cancelled_at` clear, no entries, show not past — derived, so it stops being true the moment they enter a class and there is no row to remember to close. It **prompts rather than warns**: plenty of people book stalls now and enter at the desk on the day. A cancelled registration is excluded, because they finished and then withdrew, which is a different thing to tell somebody.

**Started and never signed up.** Nothing per-show is written until `PUT /signup`, so this left no record anywhere. `show_registration_drafts` is that record — one row per (show, exhibitor), `started_at` and `last_opened_at` and nothing else.

| | |
| --- | --- |
| Written by | `POST /shows/{id}/register/draft`, fired by `RegisterShowForm` on mount. Idempotent; only moves `last_opened_at` on a return visit |
| Deleted by | `PUT /signup` (in the same transaction as the row that supersedes it) and `DELETE /shows/{id}/register/draft`, which is the card's *Not entering this show* |
| Read by | `GET /my-shows/` → `started`, a **separate array** from `shows`: a draft has no bill, no back number and no entries, and folding it in would mean a card of zeroes reading as a $0.00 registration — `shows` is also what the profile's Show History tab renders as shows the exhibitor competed in |
| Filtered to | PUBLISHED shows with no `show_entries` row. Registration closes with that status, so a bookmark on anything else points at a form that would turn them away; and any `show_entries` row already puts the show in `shows` with a record of its own |

**Not a `show_entries` shell row**, which is the obvious place to put it and the wrong one. A NULL `registered_at` row is what the office creates while adding a late entry by hand, and it is what the desk draws its roster from — writing one when somebody merely opens a form would fill the roster with window-shoppers and inflate the "no back number" and "no entries" counts the office works down. The desk's roster is people the show is expecting; this is people who looked.

**No progress column.** How far they got is derived on read from `profile_checklist` and their horse count — the same answers the registration screen itself renders and `PUT /signup` refuses on, so the card cannot claim somebody is further along than the endpoint thinks. A stored step would have gone stale the first time they completed their profile from `/profile` instead. What is *not* derivable, and is the whole content of the row, is that they were looking at this show.

**The dismiss is not decoration, and it deletes.** The bookmark is written when the screen opens, so somebody who clicked Register to see what the classes cost gets one too; without a way to say no, My Shows fills with shows they are not entering and stops being read. Deleting rather than marking is deliberate — opening the form again is somebody changing their mind, and the honest answer then is to bookmark it again rather than to remember that they once said no.



### An Automatic Charge May Name Its Classes

`show_fee_classes` (migration 137). **No rows means the whole schedule** — the ordinary case, and what every fee at every show had before this. Rows narrow the charge: every count it multiplies is taken over those classes alone, and an exhibitor who entered none of them owes nothing for that line.

It exists because "not a club-sanctioned class" (migration 131) was the only scope the app had, and a real catalogue is drawn far finer. The MNSPHC bill carries an *APHA Youth class* rate, an *All Breed WSCA* rate, an *MNSPHC All Breed* rate and two All Day fees split Open/Amateur against Youth. All five are automatic units, so all five billed every exhibitor whatever they entered — one $36 amateur class came out at **$552**.

- Set in the **Class Fees** box (`ShowChargesEditor`) under *Choose which classes use the fee*. The endpoint refuses a list on a reserved fee (billed from a booked quantity) or a flat price (billed to nobody).
- **Every fee quoted "per class" offers the list, with every class ticked by default** — `per_entry`, `per_judge_per_entry`, and a leftover `per_class_per_horse` row (`billing.PER_CLASS_FEE_UNITS`). **Club-sanctioned and futurity classes are never offered**, and billing leaves them out of both per-class charges, so *Select all* charges exactly what the list shows. The manager narrows a fee by unticking. The list opens as soon as a per-class unit is picked (new fee, or a saved fee's unit changed); a fee that loads already per class starts closed. Every class ticked is saved as no rows, so it keeps covering classes added later; ticking the last unticked box goes back to that. Every class unticked cannot be saved, and stops the step's Next with the error rather than being dropped.
- On `per_entry` and `per_judge_per_entry` the list narrows the charge, and the show bill prints it under a narrowed fee (*Classes 1–3, 7*); a fee on every class prints nothing extra. A leftover `per_class_per_horse` row bills nobody and says so on the row.
- **Boarding Fees no longer offers the per-class units or lists those rows** — they are in the Class Fees box, beside the list that may narrow them.
- A per-horse or per-exhibitor charge offers no list. A row that already carries one from before keeps it on screen until cleared, because billing still honours it.
- **Narrows, never widens.** Migration 131's exclusion still applies on top, so ticking a WSCA class into the breed body's own assessment does not make the assessment reach it.
- `charge_lines` reports each line's **own** counts (`horse_count`, `entry_count`, `is_scoped`), not the exhibitor's, or the arithmetic printed on the bill would not foot.

**Two things scoping does not fix, and both were part of the same $552.** A class rate stated both on the class row and as a *per class* fee charges twice — `per_entry` bills now — so a class is priced in one place or the other. And an **All Day fee is an elective bundle** that replaces per-class fees rather than adding to them; nothing stored says whether an exhibitor chose it, so it is `flat` and bills nobody until the app models the election. See the Sharp Edge in `Claude.md` on units moving between billing families.
...
Side pots are money pools that span multiple classes within a show - analogous to division jackpots run alongside pattern-class divisions like Showmanship 13&U / 14-17 / 18-34 / 35-49 / 50+. The pot is the show's to run, and a class it bundles carries the buy-in: **entering one of a pot's classes means being in the pot** (*A Side Pot's Classes Come With The Buy-In*, below). What is elective is which classes the show bundles — an exhibitor who enters none of them owes nothing.
...
### A Side Pot's Classes Come With The Buy-In

**Entering a class an open pot bundles requires being in that pot.** Both entry doors enforce it and refuse with `409 SIDE_POT_REQUIRED`; the rule is [backend/side_pot_membership.py](../backend/side_pot_membership.py), read by `POST /shows/{id}/classes/{cid}/entries` and `POST /shows/{id}/register`. Bundling a class is the show saying the jackpot is part of it — before this, being in a pot was purely elective, the desk ticked people in afterwards, and an exhibitor could not join one at all, so a show running a jackpot over its ranch riding classes had no way to say so.

The shape of the rule is where the care is:

- **At least one covering pot, never all of them.** A class two open pots have bundled is a choice the show meant somebody to make — a $25 jackpot and a $100 jackpot over the same classes is a real thing — and requiring both would charge two buy-ins for one entry. With one covering pot, which is the ordinary case, the readings coincide.
- **A `settled` or `closed` pot never obliges.** `POST .../side-pots/{id}/entries` refuses to add to a settled pot, so requiring it would leave the class unenterable by any route — a refusal nothing at the show could clear, which is exactly the case of a late entry into a class whose pot has already paid out.
- **Membership is per `show_entries` row, not per class entry.** One buy-in covers every class the pot names, so the second class in it asks for nothing.
- **The opt-in travels with the entry.** `side_pot_ids` on the entry request buys in inside the same transaction, because a pot joined by a separate call before a rejected entry is money owed for a class nobody is in. It is checked, not trusted: a pot from another show, a pot that is not open, or a pot covering none of the classes being entered is a 422 rather than a silent drop — a buy-in for something nobody entered is a charge the exhibitor never chose.
- **A caller with no roster row is told to fix that first** (`SHOW_ENTRY_REQUIRED`), since the buy-in hangs off `show_entries`.

### Scratching Gives The Buy-In Back

The other half of the same rule. If entering a pot's class is what buys somebody in, scratching their last class in it is what takes them out — `release_scratched_pots`, called by **both** deletion doors (`DELETE /shows/{id}/classes/{cid}/entries/{eid}` and `DELETE /shows/{id}/register/entries/{eid}`) after the delete is flushed. A rule only one door honoured would mean a buy-in that survives or not depending on who pressed the button.

It is safe to do silently because **nothing at a show is paid until it settles**: pot money rides on the exhibitor's show bill, so there is no collected payment to strand, and leaving the row would bill a jackpot to somebody who is no longer in one. Three things narrow it, each a way of not clearing a row somebody meant to keep:

- **Only pots that bundled the scratched class.** A buy-in in some other pot — including a legacy one with no entries behind it — is not this deletion's business, and clearing it as a side effect would be a surprise.
- **Only where nothing of that pot is left.** One buy-in covers every class the pot names, so scratching one of six leaves the other five backing it. The check reads the exhibitor's *whole* remaining entry list, which is what makes a second horse still entered in a pattern class count.
- **Only an `open` pot.** A settled pot's buy-in funds a pool whose payouts are written; a closed one was frozen deliberately.

The flush before the read is load-bearing: read before it, the scratched entry still counts and nothing is ever released.

### The Buy-In Is On The Bill

A pot buy-in is a line on the exhibitor's show bill (`billing.side_pot_lines`), and that is a reversal worth knowing about. Pot money used to be kept out of `build_bill` deliberately, on the grounds that folding it into a balance would make Financials disagree with the bill the exhibitor reads on My Shows — which only held while the two read different things, and both read `build_bill`.

What changed underneath that reasoning is the rule above. While being in a pot was elective, the buy-in was a decision somebody made on the pot's own screen and could see there. Now that entering a bundled class *is* the buy-in, it is a charge nobody separately chose — and a charge nobody chose has to appear on the bill they are handed. Until this it appeared on none: the desk's old Side Pots section printing `$10.00 buy-in` per pot was the only place the money was visible anywhere, so removing that section left an exhibitor committed to a charge no screen showed.

Two details decide which pots charge:

- **One buy-in per pot, however many of its classes were entered.** Membership hangs off the `show_entries` row, so a pot bundling six classes bills once.
- **Every pot they are in charges, whatever its status and whatever `paid` says.** Status decides whether a pot may *oblige* an entry; it says nothing about whether a membership already taken is owed — a pot settled on the Saturday was funded by exactly these buy-ins. And `paid` is a flag the UI never sets, so billing only the unpaid rows would move the bill by a figure nothing on any screen explains. `paid` is about the *pool*: an unpaid row is money the office is still chasing, which the bill must go on asking for and the pool must not pay out.

So on the Financials screen, each pot's `buy_ins_cents` and the pot money inside `billed_cents` are **the same money answering two questions** — what the pot will pay out, and what the exhibitor owes. They are not to be added together, and the apparent duplication is not to be resolved by removing either.

Both entry forms ask for the buy-in in the same press — see the desk's *AddEntryForm* above and `shows/[id]/register/AddClassEntry.tsx`, which names each pot with its buy-in and, on the exhibitor's side, says that the money settles with their show bill. The exhibitor's class payload carries `side_pots` per class (`joined` marking one they are already in) for exactly this; the desk's carries `class_ids` on each pot. The screens are the affordance and the endpoints are the enforcement, the same division as everywhere else here.
...
**setup Step 6** and from the **Futurities** tile on the show dashboard —
...
shape and are Step 4. So the programme is a step and the entries are not, which
...
reuses everything that already works: typed signatures at sign-up, the desk's
*Signed release on file* tick for a paper copy, and the outstanding counts on My
Shows and the desk checklist.
...
### A Nomination Comes With Its Classes

A futurity class carries `entry_fee_cents = 0` and the nomination is what prices it, so the two facts are useless apart. They were nonetheless entirely independent, and it went wrong in both directions.

A nomination with **no** futurity class still owed the office fee — `billing.futurity_lines` charges it per enrollment — for a programme the horse was judged in nothing of. And the desk, which is where the rest of that exhibitor's money is worked, could only *link out* to the futurity screen to clear it. From the other end, scratching the last futurity class left the nomination behind, still charging, with nothing to say why.

`backend/futurity_enrollment.py` closes both, deliberately in the same shape as the side pot rule above:

- **Enrolling requires a class.** `POST .../futurities/{id}/entries` and the exhibitor's `POST /register/futurities` both refuse with `409 FUTURITY_CLASS_REQUIRED` when the horse is in none of the futurity's classes.
- **The enroll form books them in the same press.** It posts each ticked class to the ordinary class-entry endpoint first — the one place an entry is validated against association rules, closed classes and any side pot buy-in it obliges — then files the nomination. **Classes first is deliberate**: the failure it can leave behind is a horse entered and unenrolled, which every desk screen already flags in as many words, where the reverse leaves the bare nomination this rule exists to remove.
- **Scratching the last of them releases it.** `release_scratched_enrollments`, at both deletion doors, beside `release_scratched_pots`.
- **The desk can withdraw one.** Each futurity line on the exhibitor panel carries a **Withdraw** with inline confirmation. The class entries stay — withdrawing a nomination does not scratch anything, it leaves those classes entered and unpriced, which the panel flags immediately underneath as an unenrolled horse.

**The key is the horse, not the roster row.** That is the one place this departs from a side pot, whose membership hangs off `show_entries`: two horses of one exhibitor in the same futurity are two nominations, and scratching one horse's last class must not take the other's with it.

Enrollments filed before this rule may still carry zero classes and bill the office fee alone. Nothing rewrites them — that is exactly what the desk's Withdraw is for.
