...
| 5. Show Fees | `/admin/shows/[id]/setup/fees` | Two structured slots in `show_fees` with codes `standard_class` / `jackpot` at the top — both `per_entry`, so both are published price-list text and neither bills (what an entry is charged comes from `classes.entry_fee_cents`, and a side pot's buy-in from the pot itself, over only the classes bundled into it). Each slot now says so on the screen. Below that, **Other fees** — `office_charge_cents` + `office_charge_basis` (`per_back_number` vs `per_horse`) on the show row renders *inside* this same box now (`ShowChargesEditor`'s `officeChargeSection` prop), because to an exhibitor's bill the office charge is one more automatic charge, not a different kind of thing; it still saves on its own, batched into this step's Save button — moving where it renders did not change when it saves. Beside it, any number of the show's own named charges, each priced per exhibitor, per horse, per judge per horse, per judge per exhibitor, or per judge per entry (migrations 112, 125) — the last is the breed body's own per-entry assessment (e.g. APHA SC-125.B) and always counts only the breed association's own classes; any of the other four may opt into the same scoping via `breed_association_only` (migration 130), a checkbox next to the unit picker. Quick-add presets fill the add-fee form with a sensible unit for the two questions this box is asked most — an office fee split by horse and by judge, and the assessment. These are the only fee units besides the office charge that actually bill, via `billing.charge_lines`; they are matched by *unit* rather than by code, because a manager names them, and they are charged automatically to everyone who has entered a class rather than booked like a stall. The same editor (`components/ShowChargesEditor.tsx`) is what `/admin/shows/[id]/fees/entry` renders — the latter's own **Class entry fees** box (below it) can also bulk-fill a default price across every still-unpriced breed-association class in one action, skipping club-sanctioned, qualification-only and futurity classes on purpose. Sanctioning per-class fees are editable here and link back to Step 3 for which clubs, and forward to Step 6's Sanctioned Classes for which classes each one approves — the amount alone does not say whether it charges anybody, so each club row also reports how many classes are marked approved. A **futurity slot used to sit here and does not any more** (migration 107): one amount cannot say that the same class costs $75, $100 or $150 depending on the entrant's category. The step links on to Step 7 instead, and offers to delete a leftover `futurity` fee row — a show carrying both bills its futurity entrants twice. |
