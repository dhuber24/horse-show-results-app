...
- **Three buttons stacked at the top, and the picker folds.** *+ Add Class*, *+ Add a Grand & Reserve Class*, then *Quick Class Picker* — a column rather than a row, in the order somebody reaches for them: naming a class is the deliberate one-off, and the picker is the bulk tool underneath. The stack is an `inline-flex` column so all three match the widest label instead of running ragged. The picker is open by default — it is what the step is for, and `DualScrollBox` caps it at 70vh — and folds so a built show's class list is readable. **Both forms open above the picker**, not below it: under a grid capped at 70vh a form was off the bottom of the screen, and the button read as having done nothing.
- **The step opens with four bullets, not a paragraph.** What the picker is for, that a day is chosen before using it, that a created class is reordered by dragging, and what *Must Qualify* is for. It replaced a block of prose that explained the grid's geometry — which the grid itself shows — and named the standard library, which nobody has to know to use it. Naming it was the only thing `standardLibraryLabel` was ever for, so the page no longer computes one — the library itself is still fetched and still merged into the axes.
...
- **Must qualify is a checkbox, and unticked is open entry.** Each row of the *Classes added* list carries it (saved optimistically, per row). It replaced a *by qualification* badge beside an *Open entry* / *By qualification* toggle link, two labels for one yes/no.
