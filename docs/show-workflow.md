...
| 5. Show Fees | `/admin/shows/[id]/setup/fees` | Everything that is a class fee lives in one **Class Fees** box (`components/ShowChargesEditor.tsx`) — no class fee is set up anywhere else. There is no dedicated "Standard class fee" slot any more; a show that priced one keeps the row, now just an ordinary, renamable line in the list. **+ Add a fee** opens a blank row **above the list**, and the row it creates lands at the top too (`sort_order` below every existing row) — a new fee appearing at the foot of a dozen others is one nobody sees they just made. Every row carries a **Note for the show bill**, printed under that fee on the published bill: it is where a scope or an arithmetic the row cannot express itself gets written down. What a row actually charges ("$45 × 4 judges = $180 for each horse they enter") is the unit picker's tooltip, not a line of prose repeated under every fee. Quick-add presets fill a new row in for the questions this box is asked most — an office fee split by horse and by judge, the breed body's own per-judge-per-class assessment (e.g. APHA SC-125.B), an all-day pass, and a Jackpot/sidepot fee (`per_entry` — published on the bill only; the buy-in that actually bills is set on the side pot itself, over only the classes bundled into it). Named charges are priced per exhibitor, per horse, per judge per horse, per judge per exhibitor, or per judge per class (migrations 112, 125; the unit is still `per_judge_per_entry` internally) — every one of those five now counts only the breed association's own classes, unconditionally, the same way the assessment always did; there is nothing to opt into or check. These automatic units are the only fee lines besides the office charge that actually bill, via `billing.charge_lines`; matched by *unit* rather than by code, because a manager names them, and charged to everyone who has entered a class rather than booked like a stall. The office charge is simply one of these rows (migration 132): it was `office_charge_cents` + `office_charge_basis` on the show row, with a control of its own, until the column became an ordinary `per_exhibitor` / `per_horse` fee. It is renamable and removable like any other, and a new show adds one from the quick-add presets. The same editor is what `/admin/shows/[id]/fees/entry` renders, folded into that screen's single outer box alongside the **per-class pricing** table — the latter can bulk-fill a default price across every still-unpriced breed-association class in one action, skipping club-sanctioned, qualification-only and futurity classes on purpose. Sanctioning per-class fees get their **own box, Club Sanctioned Fees** — a different body's money on a different set of classes — linking back to Step 3 for which clubs, and forward to Step 6's Sanctioned Classes for which classes each one approves; the amount alone does not say whether it charges anybody, so each club row also reports how many classes are marked approved. It is the only thing on the step its Save button still writes, since every class fee above it saves a row at a time. A **futurity slot used to sit here and does not any more** (migration 107): one amount cannot say that the same class costs $75, $100 or $150 depending on the entrant's category. The step carries **one line saying futurities are Step 7** and nothing else about them — it used to hold a box of its own, explaining what a futurity is and offering a second "+ Add a futurity" door into Step 7, on a step about fees. What stays is the offer to delete a leftover `futurity` fee row, which is a `show_fees` row on this show and so is genuinely this step's business — a show carrying both bills its futurity entrants twice. |
