...
**Two things scoping does not fix, and both were part of the same $552.** A `show_fees` row whose name is a *class rate* duplicates `classes.entry_fee_cents` and should be `per_entry` price-list text — scoping it to the classes it names still charges them twice. And an **All Day fee is an elective bundle** that replaces per-class fees rather than adding to them; nothing stored says whether an exhibitor chose it, so it is `flat` and bills nobody until the app models the election. See the Sharp Edge in `Claude.md` on units moving between billing families.
