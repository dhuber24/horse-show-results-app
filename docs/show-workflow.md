...
- **There is no Side pots section on the panel.** It listed every pot with an *Add to pot* / *In the pot* toggle, and that stopped making sense the moment entering a class an open pot bundles became the buy-in (*A Side Pot's Classes Come With The Buy-In*): the toggle was a second route to what the entry form now does as part of the entry, and the more misleading one, since it could put somebody in a pot whose classes they had never entered. What remains is the count on the summary row, whose `title` names the pots and carries the sentence the section was load-bearing for — pot money settles with the show bill and is not in the billed figure beside it. Adding or removing a buy-in on its own is the pot's own **Side Pot Entries** screen, where the pot is the subject. The desk *payload* is unchanged and must stay so: `side_pots` (with `class_ids`) and each exhibitor's `side_pot_ids` are what the entry form reads to know which classes oblige a buy-in and which pots they are already in. Scratching the last class an exhibitor holds in a pot **releases the buy-in** at both deletion doors (*Scratching Gives The Buy-In Back*, below), so the section's one remaining job — undoing a buy-in whose classes are gone — is done by the scratch itself.
