...
- Set in the **Class Fees** box (`ShowChargesEditor`), offered only on an automatic unit and reading *Applies to all classes* until somebody narrows it. The endpoint refuses a scope on any other unit: a reserved fee bills from a booked quantity and a price-list row bills nobody, so neither has a count for a class list to narrow.
