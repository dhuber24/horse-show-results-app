...
- **Three buttons at the top, and the picker folds.** *Quick Class Picker*, *+ Add a class by name* and *+ Add a Grand & Reserve class* sit in one row above everything. The picker is open by default — it is what the step is for, and `DualScrollBox` caps it at 70vh — and folds so a built show's class list is readable. The two by-name buttons used to sit *under* the grid, which put them below the fold whenever it was open and moved them whenever it was not.
