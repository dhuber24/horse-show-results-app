...
`ClassWizardClient` (`/admin/shows/[id]/classes`, every show type) runs three parts — **Disciplines**, **Divisions**, **Build Classes**. Its UI contract:

- **One shape, always a sequence.** A three-card progress bar sits above every part: *Step N of 3*, the part's name, and what it holds (22 added, 172 classes). The current card is outlined; a card whose prerequisites are missing is dimmed and its tooltip says what to add first. It replaced two modes — a small stepper until all three parts had data, then an overview of three boxes each opening one part with *Back to setup overview* — which made the same parts read as a sequence on a new show and as three unrelated sections on a built one. It opens on the first empty part, and on Build Classes once disciplines and divisions exist.
- **Every part ends in a bar that names where it goes** (`StepFooter`), stuck to the bottom of the viewport: *Save 3 & continue to Divisions →*, *Continue to Build Classes →*, *← Back to Divisions*, and on Build Classes *Done — continue to Step 5: Sanctioning →*, labelled from `buildSteps` so it follows the wizard's order. The standard libraries and the class grid are long enough to push a static footer out of sight. Its z-index sits above the grid's pinned header, which otherwise painted over it.
- **Build Classes puts the grid first and folds the schedule underneath it.** Clicking a `(Division × Discipline)` cell creates the class immediately. The grid scrolls inside its own box (capped at 70vh) so the **discipline row and the division column stay pinned** while it scrolls — `position: sticky` only pins to the nearest scrolling ancestor, and a box that scrolls sideways already is one — and a **second horizontal scrollbar runs along its top edge**, kept in step with the box's own (`DualScrollBox`), because the bottom one is below the fold on any grid wide enough to need it. Cells use separate borders: a collapsed border belongs to the table and scrolls away from a sticky cell.
- **Add a class by name** sits under the grid, with an *Add a Grand & Reserve class* shortcut beside it. The grid names every class "{Division} {Discipline}", which no championship class is, so there was nowhere to add one. The form takes discipline, division, day, the name, **where in the day it runs** (after any class that day — a Grand & Reserve class belongs straight after the classes it calls back from, and dragging it there down a hundred-row list is the job nobody finishes) and **Must qualify**. The shortcut opens on Halter with the name started, Must qualify ticked, and the class placed after the last class that day in the same cell. A qualifying class does not mark its grid cell ✓, so adding one never hides the ordinary class for that cell. **Saving closes the form**, opens the class list, clears any search that would have hidden the new row and scrolls to it — the point of the form is the class it produces, and it used to stay open over a folded list reporting the save in a line of text.
...
- The finish button is disabled while grid creates are still draining, so nobody leaves mid-queue.
