...
| `/shows/[id]/register` | **My Registration** — everything an exhibitor signs up for at one show, as a five-step wizard on one route: a `RegistrationStepper` across the top, five `RegistrationSection` boxes (*Your details*, *Your horses*, *Stalls, shavings & camping*, *Classes & back number*, *Futurities*), Back/Next along the bottom of each, and the running bill under all of it. One route rather than five, unlike the show-setup wizard it mirrors: an exhibitor enters a show in a sitting on a phone while watching a bill, and five routes would put a page load between every answer. Each step is **locked shut** until the one above it is done and names the section to fill in first — both locks mirror a 409 the backend raises anyway (`PROFILE_INCOMPLETE`, `SHOW_SIGNUP_REQUIRED`), so the screen is never the thing doing the refusing. Opens on whichever step still needs doing; the classes step carries a *Finish later from My Shows* link, since that is the step people legitimately leave half done. Below the bill, `CancelRegistration`. Reads three payloads: `/register/preview`, `/register/signup` and `/register/futurities` |
