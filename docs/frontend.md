...
| `/shows/[id]/live` | Public active-show hub: buttons for Schedule, Results, Leaderboard, Show Bill, Details. **No board tile** — the full-screen results board is staff-only and lives at `/admin/shows/[id]/board`; standing a show up on a wall is the office's call, not a spectator's. The placings are public either way, so nothing is hidden by that — `/results` is the same data |
...
| `/admin/shows/[id]/board` | **The venue's results board** — a full-screen dark display for anything from a 24" desk monitor to a 75" lobby TV. **Staff-only**, which is why it lives under `/admin`: the admin layout's role gate (ADMIN / SHOW_SECRETARY / SHOW_MANAGER) is the whole of the protection, so whoever puts the show on a wall signs in once on that TV's browser and leaves it running. What it *shows* is not privileged — posted results only, the same public `results-index` payload `/shows/[id]/results` reads — the gate is on who may stand a board up, not on what the room may read. It was public at `/shows/[id]/board` when first built. **Sized by one question, asked once.** 24" and 75" panels both report 1920x1080, so nothing measurable tells them apart; what differs is viewing distance, and the sign-maker's rule wants about an inch of letter height per ten feet of it. So the board asks where the screen is (desk / room / lobby), stores the answer in `localStorage` under `gaitdesk.board.size`, and derives everything from it through two CSS custom properties: `--u` for the results and `--uc` for the chrome. Both are in `vh`, so a 1080p and a 4K panel of the same size lay out identically; `--uc` is the smaller of the two because the show name and the clock are context a room learns once while the placings are what it is reading from thirty feet. Every size on the page — type, badges, gutters, ticker speed — is a multiple of one of them, so one answer re-proportions the board together. `?size=desk|room|hall` overrides and is remembered, so the office can bookmark one link per screen. **A screen carries four classes, not one.** One at a time answers "what won class 14" and not the question people walk up to a board with — "has mine gone up yet?" — because finding out meant waiting out every other class of the day. The unit is a **pane**: one class's placings under one judge, filling one cell of a 2×2 grid (`CLASSES_PER_SCREEN`). A panel-judged class becomes one pane per judge, each headed with the judge's **surname**, because at a quarter of the screen there is no width to stand two judges beside each other. `gridShape()` falls back to 2×1 and 1×1 so a day with two classes posted gets two half-screens rather than two quarters and a pair of holes, and it is fixed for the whole rotation — a grid that reshaped itself between screens would move a class somebody was halfway through reading. **Every card's first page comes before any card's second**: laid out card by card instead, a screen of four panes was two classes shown twice, places 1–5 beside places 6–9 of the same class, which is the opposite of the point. **This is four at every preset, and it has a cost** — the sign-maker's rule wants *fewer, larger* items the further off the reader is, so a quarter-screen pane fits about four placings at `desk`, two at `room` and one at `hall`. It says so ("top 4 of 9") rather than trailing off.

**What fits is measured, not assumed.** A `ResizeObserver` on the grid area plus pinned row heights give an exact row count per cell; nothing is truncated to fit and no row is ever sliced through the middle. Three things come off a cell before the placings do and they are **not all in the same unit**: the pane's padding (`PANE_PAD_U`) and the judge's name (`JUDGE_HEAD_U`) scale with `--u`, its header (`CARD_HEAD_UC`) with `--uc`. The header is chrome on purpose — which class this is, is context a room reads once — and sizing it off `--u` made it more than half of a quarter-screen cell at the lobby preset. Leaving the pane's padding out of the arithmetic is half a row of overcount, and half a row is exactly what gets clipped. A card deeper than its pane is paged, and when the rotation budget (`CYCLE_SLIDES` screens × `CLASSES_PER_SCREEN` panes, divided among the posted cards) cannot afford every page, the pane says "top 4 of 12". Dwell is priced per screen from every placing on it and how far away it is read — `slideMs` runs 9–34s against the 6–22s of a one-class screen, because four times the content needs longer to read. **Scoped to one day**, taken as the latest `class_date` among posted classes rather than from the viewer's clock — a board left running overnight, or a browser in the wrong timezone, must not decide what "today" means at the show. Kept current by the same `AutoRefresh` / `router.refresh()` poll every other live screen uses (12s); there is no push channel in this app and this feature did not add one. The rotation timer is keyed on numbers, not on the rebuilt slide list, so a poll that changes nothing does not snap the board back to the first class. Also carries the things a screen left running for ten hours needs: a TV-overscan safe inset, a slow `board-drift` against burn-in, a cursor that hides after three seconds, and controls (size, **full screen**, pause, step) that fade in on mouse movement — also on keys 1/2/3, **f**, space and the arrows. Full screen sits with those rather than in the header because it is one more thing you press once while standing at the screen and never again; the page is already a `fixed inset-0` panel, so it is not about layout but about the browser's own chrome, which otherwise sits over a lobby TV until somebody finds F11. A refused `requestFullscreen` rejects rather than throwing and the board carries on unchanged. The **GaitDesk logo** replaces the word that used to be the way back out: the mark plus the separate 4:1 wordmark, the pairing the navbar uses, sized by width off `--uc` so it gives space back to the placings at the lobby preset like the rest of the chrome — with a `max()` px floor under each, because the brand minimums (48px mark, 120px wordmark) are where the mane strokes and the tagline close up and a unit that scales knows nothing about them. The **marquee names the judge**: `judge_last_name` rides on the `results-index` payload beside `judge_name`, one ticker line per judge's card and never a merged one, since the app does not combine cards into an official result. Escapes the app chrome by covering it: a `fixed inset-0 z-50` panel over the root layout's `<Navbar>`, rather than a second root layout, because one page does not justify restructuring every route into a group |
