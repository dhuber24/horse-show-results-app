...
| `/admin/shows/[id]` | Show management dashboard, including AQHA approval/validation card for AQHA shows. Carries a **Live Screens** tile out to `/shows/[id]/live` — the public screens are reached from the office rather than by leaving the admin area and finding the show's own page. Deliberately not status-gated, unlike Score Classes: what the rail sees is worth checking before the gates open, not only once results are going up |
