...
| `/admin/shows/[id]/board` | **The venue's results board** — a full-screen dark display for a lobby or ring-side TV. **Staff-only**, and that is the point of it living under `/admin`: the admin layout's role gate (ADMIN / SHOW_SECRETARY / SHOW_MANAGER) is the whole of the protection, so the person putting the show on a wall signs in once on the TV's browser and leaves it running. What it *shows* is not privileged — posted results only, the same public `results-index` payload `/shows/[id]/results` reads — the gate is on who can stand a board up, not on what the room may read. It was public at `/shows/[id]/board` when first built; the move is the only thing that changed. Not a second results page: `/results` is the searchable index somebody reads on a phone, and this is read across an arena by people who are not holding anything. So it is the same data with the opposite controls — nothing to tap, and a rotation instead. The main stage carries one class at a time, newest-posted first, for ten seconds each, splitting into a column per judge on a panel-judged class; a marquee along the bottom crawls the top placings of every class posted that day. **Scoped to one day**, and the day is taken as the latest `class_date` among posted classes rather than from the viewer's clock — a board left running overnight, or a browser in the wrong timezone, must not decide what "today" means at the show. Kept current by the same `AutoRefresh` / `router.refresh()` polling every other live screen uses (12s) — there is no push channel in this app and this feature did not add one. The client component re-renders from fresh props, so the marquee is not restarted by a poll. Escapes the app chrome by covering it: a `fixed inset-0 z-50` panel over the root layout's `<Navbar>`, rather than a second root layout, because one page does not justify restructuring every route into a group |
