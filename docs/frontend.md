...
- **An "this may not apply to you" goes above the form, not below it.** `StepLayout`'s `skip` renders between the tab bar and the step's content, because the person who needs it is the one for whom the whole screen is irrelevant — at the bottom they find out only after reading it all for something to fill in. Its label names the *answer* ("Skip — no club sanctioning"), not the act of leaving; bare *Skip* reads as putting a job off rather than as recording that there is none. It is offered only while the step is empty, and only where empty is a supported answer.
...
### A Navigational Tab Bar Is A `<nav>`, Not A `role="tablist"`

The show setup wizard's nine steps render as tabs (`admin/shows/_wizard/WizardStepper.tsx`), and each one is a **route**. So the markup is a `<nav>` of links with `aria-current="step"` on the active one — never `role="tab"` / `role="tablist"` / `aria-selected`, which promise a screen reader a tab-panel relationship that does not exist when the press navigates away. `scribe/JudgeTabs.tsx` is the other half of the rule and *does* use the tab roles, correctly: it swaps a panel in place without leaving the page. Which set of semantics applies is decided by whether the press is a navigation, not by how the control looks.

The steps **wrap** rather than scrolling horizontally. Nine labelled steps is about 1,000px of tabs, so a single `overflow-x-auto` line put steps five onwards off the right-hand edge with nothing to say they were there. Two rows on a laptop and four on a phone is the cost of every step being one press from every other. It is `sm:sticky sm:top-0` — pinning a four-row bar to the top of a phone is half the viewport, and the site header is static so at desktop widths there is nothing at `top: 0` to collide with.

**A helper shared between a server component and a client one goes in the plain module.** `stepName()` sat in `WizardStepper.tsx` (`'use client'`) and was called by `StepLayout` (a server component); `tsc` and ESLint both passed, and the page rendered an error boundary reading *"Attempted to call stepName() from the server"*. It lives in `_wizard/steps.ts` now. The type imports across that boundary are fine — they are erased.

### The Fee Schedule Is Grouped

`ShowbillDocument`'s **Fees** section reads its groups from `FEE_GROUPS` / `groupFees` in `frontend/lib/fee-units.ts` — *Show office*, *Stalls & bedding*, *Camping & hook-ups*, *Added to every entry*, *Other charges* — and prints a line under each heading saying whether the amounts below are the reader's to control.

A flat list could not be read as "what will this weekend cost me". A bag of shavings you order, a per-judge assessment that simply arrives on the bill, and a stall-cleanout penalty the app never charges anybody all render as "$25 (per …)", and telling them apart needs the billing families from `billing.py` that the exhibitor has no way of knowing. The groups *are* those families — reserved, automatic, and neither — split once more where the exhibitor's decision differs (stalls are ordered by the stall, camping by the night, day or spot).
...
### The Bill's Class Fees Line Opens, And Its Sub-Lines Have To Foot

`ShowBillBreakdown`'s **Class fees** row is a disclosure (`▸`/`▾`, `aria-expanded`, the same idiom as `ClassWizardClient` and `AccountsPanel`): one sub-line per class, with its number, name, horse and amount. `detailed` no longer renders a separate table — it only decides whether the row **starts** open, which is what that prop was always reaching for. The card wants a total and a shape; somebody who clicked through to "what do I owe" wants the line that surprised them already in front of them.

**The sub-lines carry `fee_cents` alone, never `fee_cents + sanction_cents`.** As a separate "Classes entered" table above the summary the combined figure was merely a different question, and it did not add up to the Class fees total printed below it. As sub-lines *under* that total they have to, and a bill whose own rows do not foot is the one thing an exhibitor checking it will not forgive. A class carrying club money says `+ $X club sanction` on its own line, and that money is totalled in **Club sanction fees** below, where `class_sanction_total_cents` already put it.

They are `dt`/`dd` pairs in the same `grid-cols-2` as every other row rather than a nested table, so each class's amount sits in the same column as the total it is part of. That is the whole point of breaking the line open, and it is what an indented table would lose — which is also why this is a `useState` toggle and `ShowBillBreakdown` is a client component, rather than a `<details>`/`<summary>` that would keep it server-rendered and break the grid.

The row is guarded on `class_lines.length`, **not** on `class_fee_total_cents > 0`: an exhibitor entered only in futurity classes has a $0 class-fee total and real classes to show, and hiding the row would leave the futurity charge below looking like it came from nowhere.
...
leaves it readable on a shared device after sign-out.

**Only `/_next/static/` is cache-first, and never in development** (v3). v2 cached
every script, style, image and font by `request.destination`, on the grounds that
Next content-hashes chunk filenames. A production build does — but that rule also
took in `/public` files like the logo and every `/_next/image` URL, none of them
hashed, and **`next dev` hashes nothing at all**: the fees step is
`/_next/static/chunks/app/admin/shows/[id]/setup/fees/page.js` before and after
every edit. A worker registered on `localhost` therefore served the first copy of
each page it ever fetched, reload after reload, while the dev server sent the new
one — HMR masks it inside a single open tab, so a finished change looks like one
that never shipped, and a browser driven by a fresh Playwright profile never sees
it. So the static branch now matches `/_next/static/` alone, and
`ServiceWorkerRegistration` does not register the worker unless
`NODE_ENV === 'production'` — in development it **unregisters** any worker and
deletes the `gaitdesk-*` caches a previous visit left. A browser already stuck
on a stale dev chunk is still running the old registration code, so it needs one
hard refresh (Ctrl+Shift+R, which bypasses the worker) or two ordinary reloads (the
first installs v3, whose activate drops v2's cache).
...
| `/shows/[id]/live` | Public active-show hub: buttons for Schedule, Results, Leaderboard, Show Bill, Details. **No board tile** — the full-screen results board is staff-only and lives at `/admin/shows/[id]/board/results`, reached through the Live Screens page at `/admin/shows/[id]/board`; standing a show up on a wall is the office's call, not a spectator's. The placings are public either way, so nothing is hidden by that — `/results` is the same data |
...
| `/shows/[id]/details` | Show details: venue, dates, show type, **club sanctioning**, shavings policy, and links to the show bill and the show office. **The show bill is not on this page** — it was, embedded below the facts card, and at a show that uploaded its own bill that meant a PDF viewer and then the whole generated bill again under a second heading. Nothing on this page is reader-specific: the balance and the link to your own entries both moved off it, to the **What I Owe** tile and the registration screen |
| `/shows/[id]/showbill` | The show's published show bill, and the only page it is on. For most shows that is the **printable** generated one — `ShowbillDocument`, with its masthead and a print stylesheet. A show that uploaded its own (Setup Step 8) gets `UploadedShowbill` first — the file in place, with Download and Open buttons and its upload date — **and the generated bill beneath it** under **Classes, judges and fees as entered in this app**: that is drawn from the fee list the app charges from, so an uploaded PDF must not be able to hide it. (Show Details carried that guarantee while it printed the bill; it moved here with the bill.) The branch is on `effective_source`, never the show's stored choice, so a show pointed at a file that is not on record falls back to the generated bill rather than rendering an empty frame. **Download / print show bill** is `window.print()`, so every browser's Save-as-PDF produces the document; a second button saves the class list as CSV for spreadsheet work — `ClassListCsvButton`, which rides on the uploaded view too, since the export is about the schedule in this app rather than about which bill the show publishes. A **Show Bill** tile on the exhibitor hub, the signed-out show page's one link, and linked from `/live`, the registration screen and the foot of Show Details. The futurity card on the registration screen links to `#futurities` here, which resolves under an uploaded bill too |
| `/shows/[id]/my-bill` | One show's itemized bill for the signed-in exhibitor — Class fees (expandable to a sub-line per class, with horse names; open by default here via `detailed`), reservations, the show's automatic charges, total. Reads `GET /my-shows/` via `loadMyShowBill()` and picks this show out of it, so the number is byte-for-byte the one on My Shows. The headline is the shared `DueAtShow`, the same component Show Details puts above the link down to here |
| `/shows/[id]` | **Signed out:** event details, a link to the **Show bill** (public, and the thing somebody reads to decide whether to enter at all), then two actions — *Register for this show* and *Contact show staff*. No class schedule; someone deciding whether to enter does not need a wall of class numbers, and the full one is on the show bill anyway. **Signed in and unable to score:** `ExhibitorShowHub` — `ExhibitorStatusBanner` over tiles, ordered about-me first: Sign Up / My Registration, **What I Owe** (only with a standing at this show, and not gated on registration being open — the bill outlives it), then Class Schedule, Show Bill, Show Details, Results, and Message the Show Office. **ADMIN / SCRIBE:** the class list, because for them each row is a link into a scribe screen. The "Read-only — results can only be entered when the show is Active" banner is shown **only** to ADMIN / SCRIBE; an exhibitor or spectator reading the class schedule has no scoring screen to be locked out of |
...
| `/shows/[id]/signup` | Exhibitor show sign-up — stalls, shavings, camping, via the shared `ReservationFields`. Required before class registration; forwards on to `/register` when saved. Renders `ProfileStep` instead when the caller's profile is short, so arriving by this URL directly cannot skip step one. Once signed up, also carries `WaiverSignatures` — the entry blank and releases, read and signed here. The same editor is folded into `/register`, so this route is the door people are pointed at rather than the only way in |
...
| `/shows/[id]/register` | **My Registration** — everything an exhibitor signs up for at one show, as a wizard on one route: a `RegistrationStepper` across the top, `RegistrationSection` boxes (*Your details*, *Your association memberships*, *Your horses*, *Stalls, shavings & camping*, *Futurities*, *Classes & back number*), Back/Next along the bottom of each, and the running bill under all of it. Six steps at most: memberships is dropped at a show with no breed or club affiliation and futurities at a show that runs none, so the stepper is built from data rather than a fixed list. **Futurities sit above the classes** — an enrollment adds a line to the bill and its classes are entered in the step below, so asking afterwards left a running total that was about to change. One route rather than six, unlike the show-setup wizard it mirrors: an exhibitor enters a show in a sitting on a phone while watching a bill, and separate routes would put a page load between every answer. Each step is **locked shut** until the one above it is done and names the section to fill in first — both locks mirror a 409 the backend raises anyway (`PROFILE_INCOMPLETE`, `SHOW_SIGNUP_REQUIRED`), so the screen is never the thing doing the refusing; memberships locks nothing below it, because it blocks nothing. Opens on whichever step still needs doing — never on memberships, which is optional — unless `?step=` names one, which is how the add-a-horse wizard returns somebody to the box they left. The classes step carries a *Finish later from My Shows* link, since that is the step people legitimately leave half done. On mount it `POST`s `/register/draft`, which is what puts an abandoned registration on My Shows. Below the bill, `CancelRegistration`. Reads three payloads: `/register/preview`, `/register/signup` and `/register/futurities` |
...
| `/my-shows` | Exhibitor "My Shows": itemized bill per show (classes, NSBA sanction, office charge, stalls/shavings/camping). No roll-up across shows — the office collects per show, against a back number, so the total lives on each show's Details page. Above the bills, a **Started — not finished** section: shows they opened the registration screen for and never signed up to, from `GET /my-shows/` → `started` (migration 136), each `StartedRegistrationCard` naming the step to resume at, the profile rows still outstanding, and a *Not entering this show* dismiss. A card down in the bills also says when somebody is **signed up with no classes entered** — see `hasNoClassesYet()`. This is what the navbar **My Shows** button opens |
...
| `/profile/horses/new` | Add-a-horse wizard (static segment, wins over `[id]`); seeds from `?name=` / `?association_id=&registration_number=`, and returns to `?next=` on create, link **or** cancel — sanitised by `safeNextPath`, so the registration screen can send somebody here and get them back to the step they left. Keeps a `localStorage` draft of every answer except the staged documents, restored on mount with a *Start over*; the Registrations step opens with the registry the ticked breeds imply (`lib/breed-associations.ts`) |
...
| `/api/shows/[showId]/showbill-document` | The show's own uploaded show bill: put one on file (`POST`, multipart) or take it off (`DELETE`, which also puts the show back on the generated bill and returns the resolved state) |
| `/api/show-bill-imports` | `GET` the caller's recent show bill reads; `POST` a bill (multipart) to start a read — answers 202 with a `pending` row |
| `/api/show-bill-imports/[importId]` | Poll one read; once it has succeeded, `extracted` and `resolved` for the review screen |
| `/api/show-bill-imports/[importId]/apply` | Create the reviewed show. A 422 carries every problem at once in `detail.problems` |
| `/api/show-companies` | `GET` every show company (`?user_id=` narrows to one account's), `POST` a new one. GaitDesk admin only on the backend |
| `/api/show-companies/[id]` | `GET` / `PATCH` (name, notes) / `DELETE` one company |
| `/api/show-companies/[id]/members` | `POST {user_id}` — add an account; it gets every feature the company has |
| `/api/show-companies/[id]/members/[userId]` | `DELETE` — take an account out of the company |
| `/api/show-companies/[id]/features/[feature]` | `PUT` turns a paid feature on, `DELETE` turns it off; both idempotent and answer with the whole company |
| `/api/shows/[showId]/showbill-document/file` | The uploaded file itself, streamed through — **deliberately unauthenticated**, like the generated show bill it stands in for. `?download=1` for a save rather than an in-page render. Exists only because `API_URL` names a container the browser cannot reach |
| `/api/shows/[showId]/showbill-source` | Which show bill the Show Bill button opens (`PUT`). 422s if `uploaded` is chosen with no file on record |
| `/api/my-shows` | Exhibitor's shows with itemized bills |
| `/api/shows/[showId]/marquee` | What the live screens' marquee carries (`PUT` — results, message, or both). 422s a message mode with nothing typed, or a message over 500 characters. The board reads it server-side from `GET /shows/{id}/marquee`, which is public |
| `/api/shows/[showId]/register/draft` | `POST` bookmarks a registration the caller has started; `DELETE` is the *Not entering this show* dismiss on the My Shows card. Both derive the exhibitor from the session |
...
| `/admin` | Admin landing |
| `/admin/shows` | Admin/manager/secretary show list, under the **Automate your show setup** card (`components/AutomateShowCard.tsx`) — the show bill import, sold: a live *Upload show bill & automate my show* button for a company on the plan, the same button disabled with *"You must upgrade to GaitDesk Pro to enable this feature"* for one that is not |
| `/admin/shows/new/from-showbill` | **Start a show from its show bill** — reached from the *Upload show bill & automate my show* button on `/admin/shows` and `/admin/shows/new`. Upload the club's own PDF (or a photo of a one-page bill — one file is read per upload) through a dashed drop zone with a **Choose your show bill** button, which also takes a dragged file and turns into a file card (name, size, *Change*, *Remove*) once one is chosen; a wrong type or a file over 10 MB is refused on the spot, before any upload. The model reads it in the background; the page also lists the caller's recent reads, so a read left running has a way back. Says so and links to the step-by-step setup when `ANTHROPIC_API_KEY` is not set. See [showbill-import.md](showbill-import.md) **A paid feature** (migration 142): a caller whose show company is not on the plan sees the upgrade message (`PaidFeatureNotice`, the same words as the locked button on `AutomateShowCard`) and the step-by-step link instead of the form |
| `/admin/shows/new/from-showbill/[importId]` | **Review what was read, then create.** Polls the read until it finishes; then every section is editable — show details and paperwork, venue (on file / new / none; *new* only for a manager or admin), judges (on file or new, with the name match said out loud), club sanctioning, the bill's **class prices** with the per-judge arithmetic shown, a class table per day (rate or typed price, clubs, *Must qualify*), and the other fees with the family each unit bills in. **Create draft show** makes everything in one transaction and lands on the setup hub. State and money arithmetic live in `lib/showbill-import.ts`, not the component. Gated like the upload: a company whose feature was turned off mid-review gets the same notice instead of a screen whose every call 403s |
| `/admin/shows/[id]` | Show management dashboard, including AQHA approval/validation card for AQHA shows. Carries two display tiles. **Public Results & Show Details** goes out to `/shows/[id]/live` — the public screens are reached from the office rather than by leaving the admin area and finding the show's own page, and it is named for both halves of the hub it lands on, with "Public" so it does not read as the office's own results work. The tile grid is two across and its order is its layout: Score Classes sits directly above Side Pots with Financials beside it. **Live Screens** goes to `/admin/shows/[id]/board` in a new tab (`target="_blank"`) — always the Live Screens page, never straight into a board at a remembered size — and this console is the *only* way in: the board is staff-only, so it is not a tile on the public hub. Neither is status-gated, unlike Score Classes: what the rail sees is worth checking before the gates open, not only once results are going up |
| `/admin/shows/[id]/board` | **The Live Screens page** — where the dashboard's Live Screens tile lands, every time: two buttons, **Results Board** (to `/board/results`) and **Gate Board** (disabled, coming soon, with a `title` saying so). Each screen owns its own page and its own settings, so this one stays a choice between screens rather than a form that grows a section per board — and the gate board arrives as the second card rather than a rearrangement. Same full-screen dark panel as the boards, since it is usually opened on the TV's own browser. **An old `?size=` link is forwarded** to `/board/results` with its query intact: the board ran at this address until it moved, and a TV's browser may be bookmarked on one. |
| `/admin/shows/[id]/board/results` | **The venue's results board** — a full-screen dark display for anything from a 24" desk monitor to a 75" lobby TV. **Staff-only**, which is why it lives under `/admin`: the admin layout's role gate (ADMIN / SHOW_SECRETARY / SHOW_MANAGER) is the whole of the protection, so whoever puts the show on a wall signs in once on that TV's browser and leaves it running. What it *shows* is not privileged — posted results only, the same public `results-index` payload `/shows/[id]/results` reads — the gate is on who may stand a board up, not on what the room may read. It was public at `/shows/[id]/board` when first built. **Sized by one question, asked once.** 24" and 75" panels both report 1920x1080, so nothing measurable tells them apart; what differs is viewing distance, and the sign-maker's rule wants about an inch of letter height per ten feet of it. So the board asks where the screen is (desk / room / lobby) and derives everything from the answer through two CSS custom properties: `--u` for the results and `--uc` for the chrome. Both are in `vh`, so a 1080p and a 4K panel of the same size lay out identically; `--uc` is the smaller of the two because the show name and the clock are context a room learns once while the placings are what it is reading from thirty feet. Every size on the page — type, badges, gutters, ticker speed — is a multiple of one of them, so one answer re-proportions the board together. **Opened bare, the route is the Results Board page** — the three sizes and the **Marquee** editor, with **← Live Screens** back. The answer is carried in the URL as `?size=desk|room|hall` and **never in `localStorage`**: a remembered size skipped the pages in front of the board on every later visit, and those are where the marquee is set and the gate board will be picked, so the Live Screens tile has to land on them. A reload of the TV's browser keeps `?size=`, and a link carrying it can still be bookmarked for one particular screen. Choosing a size from the page is a `history.pushState`, so Back returns to it; changing size on a running board is a `replaceState`, so Back does not step through every size tried; both are folded into Next's router, so `useSearchParams` follows with no server round trip (the page wraps the board in `<Suspense>` for it). The board's fade-in controls carry **‹ Settings** to get back to the Results Board page. **Screen time is the office's to set** (`?every=`), from a menu in those controls beside pause and step: **Auto** — the priced dwell below — or 10–50 seconds in tens, then 1–10 minutes in whole minutes (`EVERY_CHOICES`). A chosen time is taken exactly, with no pricing and no preset factor on top, because somebody who picked "30 sec" is timing it. It lives in the URL beside `?size=` for the same reasons — it belongs to this screen, it has to survive the TV's browser reloading, and it must not follow the office to the next show — and URL changes touch only the key they are about, so it survives a trip to Settings and back. A menu rather than a stepper, because the useful jump is from seconds to minutes; the controls stay up while it has focus (the browser draws the open menu itself, so moving over it reaches no handler and they would fade from under the choice), it hands focus back once a time is picked so the arrow keys and space go back to stepping and pausing, and the dwell bar restarts with the new time rather than finishing at the old speed. **A screen carries four classes, not one.** One at a time answers "what won class 14" and not the question people walk up to a board with — "has mine gone up yet?" — because finding out meant waiting out every other class of the day. The unit is a **block**: one class — **its header once**, ring · division · discipline, number and name, with "top 4 of 6" and any "1 / 2" — and under it **one box per judge's card**, holding only the judge's **surname** and the placings. The header used to sit inside every box, so a panel-judged class printed the same two lines four times over when only the judge differed, and each copy was height taken from the placings. A block takes as much of a 2×2 screen (`SCREEN_COLS` × `SCREEN_ROWS`) as it has judges: a single-judge class **a quarter**, a two-judge class **a row**, and a panel of three or more **the whole screen** (`screenGrid()`: three across, four two by two, more three to a row). `buildSlides` packs quarters and rows greedily and in order — a quarter drops into a hole in an earlier row rather than leaving it — and a block that needs the screen closes the one being filled first. When everything that shares a screen fits in one row, that row gets the full height (and a lone single-judge class the full width), which only makes boxes taller. **Every class's first page comes before any class's second**, and **all of a class's cards page together**, so a screen never stands one judge's first page beside another's second. **Four quarters at every preset has a cost** — the sign-maker's rule wants *fewer, larger* items the further off the reader is — so **a class that cannot fit one whole placing in its share is given the screen instead** (`spanFor`): a two-judge class at `hall` does this. A 4-judge panel fits four placings a box at `desk`, two at `room` (paged 1 / 2) and one at `hall` (1 / 4), none clipped — the lobby clipping of the pane layout went with the per-box header. **The top four places, and no further** (`TOP_PLACES`): everybody on the grounds has the whole card on their phone, so the wall is not the record, and a room reads four placings at a glance where it has to wait out twelve. *Places*, not rows — a tie for fourth shows both horses, because showing one of two tied horses is choosing between them — and only placed rows, so a disqualification or a no-score is on the phone and not in anybody's top four.

**What fits is measured, not assumed.** A `ResizeObserver` on the grid area plus pinned row heights give an exact row count per cell; nothing is truncated to fit and no row is ever sliced through the middle. The `Fit` is worked out once per stage size for every kind of block — a box in a shared row, with and without a judge's name, and a box in a block with the screen to itself, by judge count — and is **unclamped**, so "not even one row" is an answer rather than being rounded up into a row that is then clipped. Several things come off a box before the placings do and they are **not all in the same unit**: its padding (`PANE_PAD_U`) and the judge's name (`JUDGE_HEAD_U`) scale with `--u`, the class header above it (`CLASS_HEAD_UC` + `CLASS_HEAD_GAP_UC`) with `--uc`. The header is chrome on purpose — which class this is, is context a room reads once. Leaving the box's padding out of the arithmetic is half a row of overcount, and half a row is exactly what gets clipped. **Placings carry ribbons**: the SVG rosette from the class results page in the standard US colours (blue, red, yellow, white), from `components/Ribbon.tsx` so the two screens cannot drift apart. It scales with `--u` (`RIBBON_U`, inside `ROW_U`) and draws its number larger than the results page does (`numberSize`), because the results page prints the ordinal underneath and on the board the rosette is all a room has. A header says "top 4 of 12" whenever a card holds more than the block shows, counted against the whole card so somebody whose horse placed seventh knows the rest is on their phone rather than missing — and it names the last *place* shown, so a tie for fourth is five horses and still "top 4". Where even four rows are more than a box fits, the class is paged, and when the rotation budget (`CYCLE_SLIDES` screens, counted in quarters and divided among the posted classes) cannot afford every page the block shows fewer and says how many. Dwell is priced per screen from every placing on it and how far away it is read — `slideMs` prices 9–34s against the 6–22s of a one-class screen, because four times the content needs longer to read — and then adds a flat five seconds (`EXTRA_DWELL_MS`) after the clamp, so a screen is held 14–39s. Flat rather than scaled by the preset's `dwell`, so it is the same extra time on every screen, and after the clamp so the ceiling does not swallow it. **Scoped to one day**, taken as the latest `class_date` among posted classes rather than from the viewer's clock — a board left running overnight, or a browser in the wrong timezone, must not decide what "today" means at the show. Kept current by the same `AutoRefresh` / `router.refresh()` poll every other live screen uses (12s); there is no push channel in this app and this feature did not add one. The rotation timer is keyed on numbers, not on the rebuilt slide list, so a poll that changes nothing does not snap the board back to the first class. Also carries the things a screen left running for ten hours needs: a TV-overscan safe inset, a slow `board-drift` against burn-in, a cursor that hides after three seconds, and controls (settings, size, **full screen**, screen time, pause, step) that fade in on mouse movement — also on keys 1/2/3, **f**, space and the arrows. Full screen sits with those rather than in the header because it is one more thing you press once while standing at the screen and never again; the page is already a `fixed inset-0` panel, so it is not about layout but about the browser's own chrome, which otherwise sits over a lobby TV until somebody finds F11. A refused `requestFullscreen` rejects rather than throwing and the board carries on unchanged. The **GaitDesk logo** replaces the word that used to be the way back out: the mark plus the separate 4:1 wordmark, the pairing the navbar uses, sized by width off `--uc` so it gives space back to the placings at the lobby preset like the rest of the chrome — with a `max()` px floor under each, because the brand minimums (48px mark, 120px wordmark) are where the mane strokes and the tagline close up and a unit that scales knows nothing about them. The **marquee names the judge**: `judge_last_name` rides on the `results-index` payload beside `judge_name`, one ticker line per judge's card and never a merged one, since the app does not combine cards into an official result, carrying the same top four as the boxes. **What the marquee carries is the show office's call** (`show_marquees`, migration 139): the results, a message, or both — the message repeated after every three result lines (`MESSAGE_EVERY`) as well as at the head of the loop, because a lap is every class of the day and an announcement seen that rarely has been missed. Set on the Results Board page and **stored on the show, not in the browser**, so a running board picks a change up on its ordinary 12-second poll wherever it is — a message typed on the office laptop reaches the lobby TV without anybody walking over. The editor adopts a polled value only while nothing in it is being edited, so a half-typed message is never overwritten. Message items are set in the accent colour, so a room tells an announcement from a placing without reading it, and **a pass does not come round again until it has left the screen**: every copy of the strip ends in a band's width of blank space, and since the strip travels exactly one copy's width per lap, the next pass enters at the right edge only as the last one leaves the left. Repeating a short message until it filled the band was tried first; a line that never stops reading "Lunch until 1:00 ★ Lunch until 1:00" is read as nothing. It applies to the results too, where it marks the end of a lap. The keyboard shortcuts are ignored while typing in a field and with a modifier held — a "1" in the message must not open the board, and Ctrl+F is find. Escapes the app chrome by covering it: a `fixed inset-0 z-50` panel over the root layout's `<Navbar>`, rather than a second root layout, because one page does not justify restructuring every route into a group |
...
| `/admin/shows/[id]/setup/judges` | **Setup Step 2** — picks judges from the `judges` registry and assigns them to this show. Offers **Skip — add judges later** while the panel is empty |
| `/admin/shows/[id]/setup/lodging` | **Setup Step 3: Lodging & Boarding** — the stall / shavings / camping fee slots and the outside-shavings policy. Offers **Skip — nothing to book here** while no lodging fee exists: a day-haul show sells none of them |
| `/admin/shows/[id]/setup/fees` | **Setup Step 7: Show Fees** — after Sanctioning and Futurities. Opens on the **Class Fees** box (`ShowChargesEditor` — **+ Add a fee**, no quick-add presets; every fee quoted per class carries *Choose which classes use the fee*, every class ticked under **Select all** by default and unticked to narrow, and no other unit shows a class list), then **Already priced in earlier steps** below it (`EarlierFees.tsx`, read-only: each club's rate and approved-class count, each futurity's categories, office fee and late fee, with links back). The Club Sanctioned Fees box went to Step 5 and the "Save & continue to Classes" button went with it: once the sanctioning amounts moved it wrote nothing, since every class fee saves a row at a time. Offers **Skip — no charges on top of the class fee** while the box is empty |
| `/admin/shows/[id]/setup/sanctioning` | **Setup Step 5: Sanctioning** — all three questions a club sanction asks, on one screen: which clubs, what each charges and how, and which classes each one approves. The rate carries a **unit** picked from the same list Step 4 offers — per class entered (the default), per exhibitor, per horse, or either of the last two times the judge panel (`show_sanctioning.fee_unit`, migration 133) — and the row states what the pair will actually charge, including that a per-judge fee charges nothing while no judges are assigned. Every unit counts that club's approved classes only. Each club's class list has two views — **All (n)** and **Ticked (n)** — with the text filter narrowing whichever is showing, so the list somebody ticked can be read back as a list instead of hunted for down a 172-row schedule. After the Class Builder because two of the three need a schedule. One Save for the step; offers **Skip — no club sanctioning** |
...
| `/admin/shows/[id]/classes` | **Setup Step 4: Class Builder** — **one screen**, led by four bullets of guidance and a vertical stack of three buttons: **+ Add Class**, **+ Add a Grand & Reserve Class**, then **Quick Class Picker** (the foldable `(Division × Discipline)` grid, open by default). Both forms open **above** the picker, since a grid capped at 70vh put anything below it off-screen. The picker's columns and rows are this show's own disciplines and divisions **merged with its show type's standard library**, alphabetically, so a show with nothing set up opens on a full grid and a clicked square creates the discipline, the division and the class together. Disciplines and divisions were two separate steps in front of the grid until they were collapsed into its axes. The grid pins its header row and division column and carries a second scrollbar along its top; **+ Add Class** (and a Grand & Reserve shortcut) for classes the grid cannot name, whose alphabetical discipline and division pickers offer the library too and each end in **+ Add a new …** (held locally, written only by the save that uses it), which closes on a save and opens the list at the new row, and refuses a name already running that day; a searchable class list with per-row ticks and one **Delete selected** (`classes/bulk-delete`), which will not sweep up a class anybody has entered; a **Must qualify** checkbox per class. APHA/AQHA standard-class import. Renders inside `StepLayout`; the route is unchanged so per-class deep links still work. It carried two notices at the top, to Sanctioned Classes and Judging Cards; both are steps of their own now (5 and 8), which the stepper shows and the Next link walks into |
...
| `/admin/shows/[id]/classes/sanctioning` | Redirects to `/setup/sanctioning` — Sanctioned Classes is part of Step 5 now |
| `/admin/shows/[id]/desk` | **Registration Desk** — one screen, one exhibitor at a time: back number, class entries, side pot buy-ins, paperwork check-in, and their running balance. The roster lists **people**: two exhibitor records sharing a name (and so two back numbers) are one row, with each record's panel stacked under it. The entry form is a class and a horse — the division comes off the class and the relationship to the owner off the horse link. **Remove from this show** deletes any registration without payments, a self-registration included. A futurity's release is a *Signed release on file* tick. Second tab is the by-class program listing, where an expanded class can be filled without leaving the screen. Replaces `/entries`, `/check-in`, and `/back-numbers`, which all redirect here |
...
| `/admin/shows/[id]/futurities` | **Setup Step 6** — futurity list and **+ Add futurity**. Renders inside `StepLayout`; the route is unchanged, so the dashboard's Futurities tile still reaches it. Offers **Skip — no futurity at this show** while the show has none: most shows run none, and a tick that never goes green otherwise reads as a job forgotten |
...
| `/admin/users/[id]` | User profile, role, password, delete controls, and AQHA workshop date tracking. A show manager or secretary also gets a **Show Companies** section: the companies the account works for and the paid features each has on, linking to the company, where membership is edited |
| `/admin/companies` | **Show Companies** (ADMIN only) — the clubs and firms that run shows. Each row carries the company's account count and **a switch per paid feature** (`FeatureToggle`), so a feature is turned on without opening the company; *Add Company* takes a name and admin-only notes, then opens the company. Companies with somebody **asking to join** sort first and carry a badge; an independent person's own company is tagged **Independent** and shows their email; a search (company, person or email) and a filter (organizations, independents, asking to join) keep one row per independent manager and secretary from burying the clubs |
| `/admin/companies/[id]` | One company: **Paid features** (the same switch as the list, with the plan, who turned it on and when), **Accounts** (add a show manager or secretary from a picker, remove), the name and billing notes, and delete. See `backend/show_companies.py`. **Asked to join** (sign-ups that typed this company's name) comes first, with *Add to company* and *Decline*. An independent's own company says so, and offers no Remove on its owner |
| `/register/show-manager`, `/register/show-secretary` | Show-office account registration: name, email, password and an optional **Company / Organization** (migration 143). Blank gives the account a company of its own under the person's name; a new name creates that organization; an existing one becomes a request to join it for a GaitDesk admin to approve. Both redirect to `/welcome` |
...
The rosette (`components/Ribbon.tsx`, shared with the Live Screens results
board) has its petal coordinates **rounded** at module scope. Raw
...
- `lib/my-shows.test.ts` — `formatMoney` (including a negative, because a credit balance renders),
  `ordinal` (the 11th/12th/13th boundary), `formatDateRange`, `isPastShow`, and
  `hasNoClassesYet` — signed up with nothing entered, which is a prompt and not a warning,
  and is not true of somebody who cancelled or of a show that has finished.
- `lib/breed-associations.test.ts` — the breed → registry match the add-a-horse wizard
  prefills from. Pins both directions: "American Paint Horse" reaches APHA, and nothing
  reaches Foundation Quarter Horse Registry or any club. A wrong prefill costs a click,
  which is the only reason matching on a name is acceptable here at all.
