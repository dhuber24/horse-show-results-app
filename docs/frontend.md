...
| `/shows/[id]/register` | **My Registration** — everything an exhibitor signs up for at one show, in three collapsible `RegistrationSection`s over the running bill, **in order**. *Your profile* (`ProfileStep`) holds the backend's own checklist and an in-place personal-details form; *Stalls, shavings & camping* holds the shared `ReservationFields`; *Classes & back number* holds the back-number request, the entered-class table, the desk's one-class-at-a-time entry form, and the horses needing health records. Each step is **locked shut** until the one above it is done, and names the section to fill in first — both locks mirror a 409 the backend raises anyway (`PROFILE_INCOMPLETE`, `SHOW_SIGNUP_REQUIRED`), so the screen is never the thing doing the refusing. Opens on whichever step still needs doing. Below the bill, `CancelRegistration` — the exhibitor's own cancel up to a fortnight out, and the show office's contact link inside that window. Reads two payloads: `/register/preview` and `/register/signup` |
