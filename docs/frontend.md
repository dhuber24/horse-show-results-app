...
| `/admin/shows/[id]/classes` | **Setup Step 4: Class Builder** — three parts, Disciplines → Divisions → **Build Classes**, under an always-visible progress bar; the class grid pins its header row and division column and carries a second scrollbar along its top; **Add a class by name** (and a Grand & Reserve shortcut) for classes the grid cannot name; a **Must qualify** checkbox per class. APHA/AQHA standard-class import. Renders inside `StepLayout`; the route is unchanged so per-class deep links still work. It carried two notices at the top, to Sanctioned Classes and Judging Cards; both are steps of their own now (5 and 8), which the stepper shows and the Next link walks into |
