...
| `/admin/shows/[id]/setup/fees` | **Setup Step 7: Show Fees** — after Sanctioning and Futurities, so it opens with **Already priced in earlier steps** (`EarlierFees.tsx`, read-only: each club's rate and approved-class count, each futurity's categories, office fee and late fee, with links back), then the **Class Fees** box (`ShowChargesEditor`). The Club Sanctioned Fees box went to Step 5 and the "Save & continue to Classes" button went with it: once the sanctioning amounts moved it wrote nothing, since every class fee saves a row at a time. Offers **Skip — no charges on top of the class fee** while the box is empty |
