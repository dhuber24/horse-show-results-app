...
- **Add a Horse is a five-step wizard on its own page**, `/profile/horses/new` (`AddHorseWizard.tsx`): `Owner` → `Horse` → `Trainer` → `Registrations` → `Review`. Only the first two gate creation — a horse needs a name and an owner and nothing else — so `Trainer` and `Registrations` show a **Skip** button, and Review lists every omitted field as *Skipped*. The step indicator jumps back to any cleared step, and `handleCreate` re-validates every step (not just the walk-forward) because an earlier answer may have been revised.
