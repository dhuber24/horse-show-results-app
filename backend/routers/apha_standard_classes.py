"""APHA's approved class codes.

Kept at its own path because the APHA picker and the frontend proxy already
point here. The rows now come from the shared catalog view rather than a
per-association table — see migration 114.
"""
...
async def _show_type_id(db: AsyncSession):
    ...
    if show_type_id is None:
        raise HTTPException(404, "APHA show type not found")
    return show_type_id
