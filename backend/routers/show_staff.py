...
from models import Show, User, ShowAdmin, ShowScorekeeper
...
async def _assert_show_admin_access(show_id: UUID, x_user_id: str, x_user_role: str, db: AsyncSession):
    """Raises 403 unless caller is ADMIN or an assigned Show Admin of this show."""
    if x_user_role == "ADMIN":
        return
    if x_user_role == "SHOW_ADMIN":
        row = await db.execute(
            select(ShowAdmin).where(
                ShowAdmin.show_id == show_id,
                ShowAdmin.user_id == UUID(x_user_id),
            )
        )
...
async def list_show_admins(show_id: UUID, db: AsyncSession = Depends(get_db)):
    await _get_show_or_404(show_id, db)
    result = await db.execute(
        select(User)
        .join(ShowAdmin, ShowAdmin.user_id == User.id)
        .where(ShowAdmin.show_id == show_id)
...
async def add_show_admin(show_id: UUID, body: dict, db: AsyncSession = Depends(get_db)):
    ...
    existing = await db.execute(
        select(ShowAdmin).where(ShowAdmin.show_id == show_id, ShowAdmin.user_id == user.id)
    )
    if existing.scalar_one_or_none():
        raise HTTPException(409, "User is already an admin of this show")
    db.add(ShowAdmin(show_id=show_id, user_id=user.id))
    await db.commit()
    return user
...
async def remove_show_admin(show_id: UUID, user_id: UUID, db: AsyncSession = Depends(get_db)):
    row = await db.execute(
        select(ShowAdmin).where(ShowAdmin.show_id == show_id, ShowAdmin.user_id == user_id)
    )
...
async def add_show_scorekeeper(
    ...
    SHOW_ADMIN can only assign scorekeepers they created (i.e., where the scorekeeper
    ...
    if x_user_role == "SHOW_ADMIN":
        ...
        if existing_assignments.scalar_one_or_none():
            raise HTTPException(403, "Show Admins can only assign scorekeepers they created")
